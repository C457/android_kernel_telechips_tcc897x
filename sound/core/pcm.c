...
//buffalo+
static void snd_pcm_substream_proc_dai_reg_read(struct snd_info_entry *entry,
		struct snd_info_buffer *buffer)
{
	struct snd_pcm_substream *substream = entry->private_data;
	struct snd_pcm_info *info;
	...
	device = info->device;
	//device = snd_iprintf(buffer, "device: %d\n", info->device);
	...
	unsigned int DAI_BASE_ADDR = 0x76001000 + 0x81000000;
	volatile unsigned int *pDAI_DAMR = (volatile unsigned *)(DAI_BASE_ADDR + n + 0x40);
	volatile unsigned int *pDAI_MCCR0 = (volatile unsigned *)(DAI_BASE_ADDR + n + 0x48);
	volatile unsigned int *pDAI_MCCR1 = (volatile unsigned *)(DAI_BASE_ADDR + n + 0x4C);

	snd_iprintf(buffer, "DAMR: %08X\n", *pDAI_DAMR);
	snd_iprintf(buffer, "MCCR0: %08X\n", *pDAI_MCCR0);
	snd_iprintf(buffer, "MCCR1: %08X\n", *pDAI_MCCR1);
...
static void snd_pcm_substream_proc_adma_reg_read(struct snd_info_entry *entry,
		struct snd_info_buffer *buffer)
{
	struct snd_pcm_substream *substream = entry->private_data;
	struct snd_pcm_info *info;
	...
	unsigned int ADMA_BASE_ADDR = 0x76000000 + 0x81000000;
	volatile unsigned int *pADMA_TRANSCTRL = (volatile unsigned *)(ADMA_BASE_ADDR + n + 0x38);
	volatile unsigned int *pADMA_RPTCTRL = (volatile unsigned *)(ADMA_BASE_ADDR + n + 0x3C);
	volatile unsigned int *pADMA_CHCTRL = (volatile unsigned *)(ADMA_BASE_ADDR + n + 0x78);

	snd_iprintf(buffer, "TRANSCTRL: %08X\n", *pADMA_TRANSCTRL);
	snd_iprintf(buffer, "RPTCTRL: %08X\n", *pADMA_RPTCTRL);
	snd_iprintf(buffer, "CHCTRL: %08X\n", *pADMA_CHCTRL);
...
//buffalo+
	if ((entry = snd_info_create_card_entry(card, "dai_reg", substream->proc_root)) != NULL) {
		snd_info_set_text_ops(entry, substream,
							  snd_pcm_substream_proc_dai_reg_read);
		if (snd_info_register(entry) < 0) {
			snd_info_free_entry(entry);
			entry = NULL;
		}
	...
	if ((entry = snd_info_create_card_entry(card, "adma_reg", substream->proc_root)) != NULL) {
		snd_info_set_text_ops(entry, substream,
							  snd_pcm_substream_proc_adma_reg_read);
		if (snd_info_register(entry) < 0) {
			snd_info_free_entry(entry);
			entry = NULL;
		}
