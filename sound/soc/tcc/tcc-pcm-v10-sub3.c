/*
 * linux/sound/soc/tcc/tcc-pcm-v10-sub3.c
 *
...
#undef alsa_dbg
#if 0
#define alsa_dbg(f, a...)  printk("== alsa-debug PCM V10 SUB3 == " f, ##a)
#else
#define alsa_dbg(f, a...)
#endif
...
static int I2S_SLAVE_SUB3_En;
#endif

#if defined(CONFIG_SND_SOC_DAUDIO_FIFO_CLEAR_DELAY)
// I2S FIFO clear for 1st pop-noise after boot
static int first_init_delay_sub3 = 0;
static int first_init_delay_ms_sub3 = 0;
#endif
...
{
#if defined(CONFIG_ARCH_TCC897X) || defined(CONFIG_ARCH_TCC570X)
	volatile PADMA1 pADMA = (volatile PADMA1)tcc_p2v(BASE_ADDR_ADMA3);	// Planet 20120531
#else	//for 896x
...
{
#if defined(CONFIG_ARCH_TCC897X) || defined(CONFIG_ARCH_TCC570X)
	volatile PADMA1 pADMA = (volatile PADMA1)tcc_p2v(BASE_ADDR_ADMA3);
	volatile PADMASPDIFTX pADMASPDIFTX = (volatile PADMASPDIFTX)tcc_p2v(BASE_ADDR_SPDIFTX3);
#else	// for TCC896X
...
static int tcc_pcm_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
...
#if defined(CONFIG_ARCH_TCC897X) || defined(CONFIG_ARCH_TCC570X)
	volatile PADMA1    pADMA     = (volatile PADMA1)tcc_p2v(BASE_ADDR_ADMA3);		// Planet 20120531
	volatile PADMADAI1 pADMA_DAI = (volatile PADMADAI1)tcc_p2v(BASE_ADDR_DAI3);	// Planet 20120531
	volatile PADMASPDIFTX pADMA_SPDIFTX = (volatile PADMASPDIFTX)tcc_p2v(BASE_ADDR_SPDIFTX3);
#else	// for TCC896X
	volatile PADMA1    pADMA     = (volatile PADMA1)tcc_p2v(BASE_ADDR_ADMA1);		// Planet 20120531
	...
	if (prtd->params == NULL) {
		...
		if ((substream->pcm->device == __SPDIF_DEV_NUM__) || (substream->pcm->device == __SPDIF_SUB_DEV_NUM__)) {
	...
	if ((substream->pcm->device == __I2S_DEV_NUM__) || (substream->pcm->device == __I2S_SUB_DEV_NUM__) || (substream->pcm->device == __I2S_SUB2_DEV_NUM__)) {
		if (chs > 2) {                                  // 5.1ch or 7.1ch
			...
			first_init_delay_ms_sub3 = ((TCC_DMA_HALF_BUFF_STEREO * 2 * 1000) + (rate - 1)) / rate + 1;
#endif
#if defined(CONFIG_SND_SOC_DAUDIO_CLOCK_ROLE_SWITCHING)
			if(I2S_SLAVE_SUB3_En) {
...
#if defined(CONFIG_ARCH_TCC897X) || defined(CONFIG_ARCH_TCC570X)
	volatile PADMADAI pDAI  = (volatile PADMADAI)tcc_p2v(BASE_ADDR_DAI3);
	volatile PADMA1   pADMA = (volatile PADMA1)tcc_p2v(BASE_ADDR_ADMA3);
#else	//for 896x
...
static int tcc_i2s_tx_enable(struct snd_pcm_substream *substream, int En)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
#if defined(CONFIG_ARCH_TCC897X) || defined(CONFIG_ARCH_TCC570X)
	volatile PADMADAI1 pDAI  = (volatile PADMADAI1)tcc_p2v(BASE_ADDR_DAI3);
	volatile PADMA1    pADMA = (volatile PADMA1)tcc_p2v(BASE_ADDR_ADMA3);
#else	//for 896x
...
static int tcc_i2s_rx_enable(struct snd_pcm_substream *substream, int En)
{
	...
	if(En) {
		...
		/* For FIFO clear */
		tca_tcc_DAI_mute(3, runtime->channels, 1);
		msleep(2);
		...
		tca_tcc_DAI_mute(3, runtime->channels, 0);
...
#if defined(CONFIG_SND_SOC_DAUDIO_FIFO_CLEAR_DELAY)
					// I2S FIFO clear for 1st pop-noise after boot
					if(!first_init_delay_sub3) {
						pDAI->DAVC = 0x10; /* Mute ON */
						ret = tcc_i2s_tx_enable(substream, 1);
						mdelay(first_init_delay_ms_sub3);
						first_init_delay_sub3 = 1;
					}
					else
						ret = tcc_i2s_tx_enable(substream, 1);
#else
...
static int tcc_pcm_open(struct snd_pcm_substream *substream)
{
	...
	alsa_dbg("[%s] open %s device, %s\n", __func__,
			 (substream->pcm->device == __SPDIF_DEV_NUM__) || (substream->pcm->device == __SPDIF_SUB_DEV_NUM__) ? "spdif" : "pcm",
...
static int tcc_pcm_close(struct snd_pcm_substream *substream)
{
	...
	unsigned int htemp_a = 0, htemp_b = 0, htemp_c = 0, ret = 0;
	unsigned long flags;
	alsa_dbg("[%s] close %s device, %s\n", __func__, (substream->pcm->device == __SPDIF_DEV_NUM__) || (substream->pcm->device == __SPDIF_SUB_DEV_NUM__) ? "spdif" : "pcm",
			 substream->stream == SNDRV_PCM_STREAM_PLAYBACK ? "output" : "input");
	...
	if (tcc_alsa_info.flag & TCC_INTERRUPT_REQUESTED) {
		if (!(tcc_alsa_info.flag & (TCC_RUNNING_SPDIF_PLAY | TCC_RUNNING_PLAY | TCC_RUNNING_CAPTURE))) {
...
static int tcc_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
	...
	{
#if defined(CONFIG_ARCH_TCC897X) || defined(CONFIG_ARCH_TCC570X)
		volatile ADMASPDIFTX *pADMASPDIFTX = (volatile ADMASPDIFTX *)tcc_p2v(BASE_ADDR_SPDIFTX3);
#else
...
int tcc_pcm_sub3_platform_register(struct device *dev)
{
	return snd_soc_register_platform(dev, &tcc_soc_platform);
}
EXPORT_SYMBOL_GPL(tcc_pcm_sub3_platform_register);

void tcc_pcm_sub3_platform_unregister(struct device *dev)
{
	return snd_soc_unregister_platform(dev);
}
EXPORT_SYMBOL_GPL(tcc_pcm_sub3_platform_unregister);
