...
int tcc_codec_dummy_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if(!startup_1st){
		ret = tca_tcc_DAO_mute(0);	// When kernel is booting, mute disable for reduce pop-noise.
...
static int tcc_audio_probe(struct platform_device *pdev)
{
	...
	ret = tca_tcc_DAO_mute(1);	// When kernel is booting, mute enable for reduce pop-noise.
