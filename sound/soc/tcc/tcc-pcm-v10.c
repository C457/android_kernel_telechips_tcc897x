...
#undef alsa_dbg
#if 0
#define alsa_dbg(f, a...)  printk("== alsa-debug PCM V10 SUB0 == " f, ##a)
#else
#define alsa_dbg(f, a...)
#endif
...
static int I2S_SLAVE_En;
#endif

// Planet 20160113 Add CDIF Bypass
int CDIF_Bypass_En;

// SPDIF_24BIT_MODE Planet_20160715
int mSPDIF_24bit_Tx_mode = 0;
int mSPDIF_24bit_Rx_mode = 0;
...
//    dma_buffer = dma_buffer * (((length) >> 4) << 8);
	...
	// SPDIF_24BIT_MODE Planet_20160715
	if(mSPDIF_24bit_Tx_mode) {
		pADMASPDIFTX->TxBuffer[0] = 0;
		pADMASPDIFTX->DMACFG &= ~(Hw12 | Hw13);
	}
	else {
		pADMASPDIFTX->TxConfig |= HwZERO | Hw8; // | Hw1; Do not set data valid register at initial time.
		pADMASPDIFTX->DMACFG |= Hw13;
		pADMASPDIFTX->TxBuffer[0] = 0;
	}
	...
	spin_lock_irqsave(&(tcc_alsa_info.slock), flags);
	if(mSPDIF_24bit_Tx_mode) {
		pADMA->ChCtrl &= ~Hw13;	// Planet_20140317
	}
	else {
		pADMA->ChCtrl |= (Hw13);
	}
...
static void set_dma_spdif_inbuffer(unsigned int addr, unsigned int length, unsigned int period, int mode)
{
	volatile PADMA pADMA = (volatile PADMA)tcc_p2v(BASE_ADDR_ADMA0);
...
//    dma_buffer = dma_buffer * (((length) >> 4) << 8);
	...
	if(mode)
		pADMA->TransCtrl |= (Hw29 | Hw19 | Hw15 | (Hw7 | Hw6));
	else
	...
	// SPDIF_24BIT_MODE Planet_20160715
	if(mSPDIF_24bit_Rx_mode) {
		pADMASPDIFRX->RxConfig |= HwZERO | Hw1 | Hw2 | Hw3 | Hw4 | Hw23;
	}
	else {
		pADMASPDIFRX->RxConfig |= HwZERO | Hw1 | Hw2 | Hw3 | Hw4;
	}
...
static int tcc_pcm_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
	...
	if (prtd->params == NULL) {
		...
		if (substream->pcm->device == __SPDIF_DEV_NUM__) {
			prtd->dma_ch = (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) ? DMA_CH_SPDIF : DMA_CH_SPDIF_IN;	// Planet 20150812 S/PDIF_Rx
		}
		else if(substream->pcm->device == __CDIF_DEV_NUM__) {
			prtd->dma_ch = DMA_CH_CDIF;
		}
		else {
	...
	if(params->reserved[SNDRV_PCM_HW_PARAM_EFLAG0] & SNDRV_PCM_MODE_CDIF) {
...
#endif
	// SPDIF_24BIT_MODE Planet_20160715
	if((substream->pcm->device == __SPDIF_DEV_NUM__) && (format == SNDRV_PCM_FORMAT_S24_LE)) {
		if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			mSPDIF_24bit_Tx_mode = 1;
		}
		else {
			mSPDIF_24bit_Rx_mode = 1;
		}
	}
	else {
		mSPDIF_24bit_Tx_mode = 0;
		mSPDIF_24bit_Rx_mode = 0;
	}
	...
	if (substream->pcm->device == __SPDIF_DEV_NUM__) {
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			set_dma_spdif_inbuffer(prtd->dma_buffer, totsize, period_bytes, 0);	// Planet 20150812 S/PDIF_Rx
		}
	}
	else if (substream->pcm->device == __CDIF_DEV_NUM__) {
		set_dma_spdif_inbuffer(prtd->dma_buffer, totsize, period_bytes, 1);
	}
	else {
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
...
//        BITCLR(pADMA->ChCtrl,(Hw24 | Hw21 | Hw20));
	...
	if (substream->pcm->device == __SPDIF_DEV_NUM__) {
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {	// Planet 20150812 S/PDIF_Rx
...
static int tcc_i2s_rx_enable(struct snd_pcm_substream *substream, int En)
{
	...
	if(En) {
		...
		/* For FIFO clear */
		tca_tcc_DAI_mute(0, runtime->channels, 1);
		msleep(2);
		...
		tca_tcc_DAI_mute(0, runtime->channels, 0);
...
static int tcc_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	volatile PADMASPDIFTX pADMASPDIFTX = (volatile PADMASPDIFTX)tcc_p2v(BASE_ADDR_SPDIFTX0);
	volatile PADMASPDIFRX pADMASPDIFRX = (volatile PADMASPDIFRX)tcc_p2v(BASE_ADDR_SPDIFRX0);	// Planet 20150812 S/PDIF_Rx
	volatile PADMA    pADMA = (volatile PADMA)tcc_p2v(BASE_ADDR_ADMA0);
	volatile PADMADAI pDAI = (volatile PADMADAI)tcc_p2v(BASE_ADDR_DAI0);
		...
		case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
			if (substream->pcm->device == __SPDIF_DEV_NUM__) {
				...
				if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
					...
					// SPDIF_24BIT_MODE Planet_20160715
					if(mSPDIF_24bit_Rx_mode) {
						pADMASPDIFRX->RxConfig |= Hw0 | Hw23;
					}
					else {
				...
				if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
					...
					// Planet 20160113 Add CDIF Bypass Start
					if(CDIF_Bypass_En == 1) {
						pADMA_CDIF->CICR |= Hw7;            // CDIF Enable
		...
		case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
			if (substream->pcm->device == __SPDIF_DEV_NUM__) {
				...
				if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
					...
					// Planet 20160113 Add CDIF Bypass Start
					if(CDIF_Bypass_En == 1) {
						pADMA_CDIF->CICR &= ~Hw7;           // CDIF Disable
						BITCLR(pDAI->DAMR, (Hw2 | Hw8));    // CDIF Monitor Mode & CDIF Clock Select
						CDIF_Bypass_En = 0;
					}
					pDAI->DAVC = 0x10; /* Mute ON */
					memset(runtime->dma_area, 0x00, runtime->dma_bytes);
...
static snd_pcm_uframes_t tcc_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	//alsa_dbg(" %s \n", __func__);
	if ((substream->pcm->device == __SPDIF_DEV_NUM__) || (substream->pcm->device == __CDIF_DEV_NUM__)) {
		ptr = (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) ? pADMA->TxSpCsar : pADMA->RxCdCdar;	// Planet 20150812 S/PDIF_Rx
	}
	else {
		if(chs == 1) {
...
static int tcc_pcm_open(struct snd_pcm_substream *substream)
{
	...
	alsa_dbg("[%s] open %s device, %s\n", __func__,
			 ...
			 (substream->pcm->device == __CDIF_DEV_NUM__ ? "cdif" : "pcm"),
...
static int tcc_pcm_close(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct tcc_runtime_data *prtd = runtime->private_data;
	volatile PADMA pADMA = (volatile PADMA)tcc_p2v(BASE_ADDR_ADMA0);	// Planet 20120531
	volatile PADMADAI pDAI  = (volatile PADMADAI)tcc_p2v(BASE_ADDR_DAI0);
	volatile PADMASPDIFTX pADMASPDIFTX = (volatile PADMASPDIFTX)tcc_p2v(BASE_ADDR_SPDIFTX0);
	volatile PADMASPDIFRX pADMASPDIFRX = (volatile PADMASPDIFRX)tcc_p2v(BASE_ADDR_SPDIFRX0);	// Planet 20150812 S/PDIF_Rx
	volatile PADMACDIF    pADMA_CDIF    = (volatile PADMACDIF)tcc_p2v(BASE_ADDR_CDIF0);
	...
	alsa_dbg("[%s] close %s device, %s\n", __func__, (substream->pcm->device == __SPDIF_DEV_NUM__) ? "spdif" :
			 (substream->pcm->device == __CDIF_DEV_NUM__ ? "cdif" : "pcm"),
			 substream->stream == SNDRV_PCM_STREAM_PLAYBACK ? "output" : "input");
	...
	if (tcc_alsa_info.flag & TCC_INTERRUPT_REQUESTED) {
		if (!(tcc_alsa_info.flag & (TCC_RUNNING_SPDIF_PLAY | TCC_RUNNING_SPDIF_CAPTURE | TCC_RUNNING_PLAY | TCC_RUNNING_CAPTURE))) {		// Planet 20150812 S/PDIF_Rx
