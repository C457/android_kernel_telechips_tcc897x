/*
 * linux/sound/soc/tcc/tcc-pcm-v10-sub2.c
 *
...
#undef alsa_dbg
#if 0
#define alsa_dbg(f, a...)  printk("== alsa-debug PCM V10 SUB2 == " f, ##a)
#else
#define alsa_dbg(f, a...)
#endif
...
static int I2S_SLAVE_SUB2_En;
#endif

#if defined(CONFIG_SND_SOC_DAUDIO_FIFO_CLEAR_DELAY)
// I2S FIFO clear for 1st pop-noise after boot
static int first_init_delay_sub2 = 0;
static int first_init_delay_ms_sub2 = 0;
#endif
...
{
	volatile PADMA pADMA = (volatile PADMA)tcc_p2v(BASE_ADDR_ADMA2);	// Planet 20120531
...
static irqreturn_t tcc_dma_done_handler(int irq, void *dev_id)
{
	...
	if((dmaInterruptSource & DMA_CH_IN) && (tcc_alsa_info.cap_del_1st == false)) {
		tcc_alsa_info.cap_intr_byte += min_period_size;
		if(tcc_alsa_info.cap_intr_byte >= tcc_i2s_fifo_size) {
			tcc_alsa_info.cap_del_1st = true;
			if(tcc_alsa_info.cap_pos_1st != 0) {
				memset(tcc_alsa_info.cap_pos_1st, 0x00, tcc_i2s_fifo_size);
			}
		}
	}
...
static int tcc_pcm_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
	...
	volatile PADMA    pADMA     = (volatile PADMA)tcc_p2v(BASE_ADDR_ADMA2);		// Planet 20120531
	volatile PADMADAI pADMA_DAI = (volatile PADMADAI)tcc_p2v(BASE_ADDR_DAI2);	// Planet 20120531
...
#if defined(CONFIG_SND_SOC_DAUDIO_FIFO_CLEAR_DELAY)
			// I2S FIFO clear for 1st pop-noise after boot
			first_init_delay_ms_sub2 = ((TCC_DMA_HALF_BUFF_MULTI * 2 * 1000) + (rate - 1)) / rate + 1;
#endif
		}
		else {                                           // 2 ch
...
#if defined(CONFIG_SND_SOC_DAUDIO_FIFO_CLEAR_DELAY)
			// I2S FIFO clear for 1st pop-noise after boot
			first_init_delay_ms_sub2 = ((TCC_DMA_HALF_BUFF_STEREO * 2 * 1000) + (rate - 1)) / rate + 1;

#endif
		}
#if defined(CONFIG_SND_SOC_DAUDIO_CLOCK_ROLE_SWITCHING)
		if(I2S_SLAVE_SUB2_En) {
...
static int tcc_pcm_hw_free(struct snd_pcm_substream *substream)
{
	...
	volatile PADMADAI pDAI  = (volatile PADMADAI)tcc_p2v(BASE_ADDR_DAI2);
	volatile PADMA    pADMA = (volatile PADMA)tcc_p2v(BASE_ADDR_ADMA2);
...
static int tcc_i2s_rx_enable(struct snd_pcm_substream *substream, int En)
{
	...
	volatile PADMA    pADMA = (volatile PADMA)tcc_p2v(BASE_ADDR_ADMA2);
	unsigned int htemp_a = 0, htemp_b = 0, timeout = 0;
	unsigned int tChCtrl = 0, tDaCdar = 0;
	unsigned int ret = 0;
	unsigned long flags;

	if(En) {
		spin_lock_irqsave(&(tcc_alsa_info.slock), flags);
		tChCtrl = pADMA->ChCtrl;
		htemp_a = tChCtrl & Hw30;
		if(htemp_a) {
			alsa_dbg("[%s] I2S RX ADMA doesn't stop yet!!\n", __func__);
			ret = 1;
		}
		...
		//This is to remove 1st part.
#if defined(CONFIG_ARCH_TCC897X) || defined(CONFIG_ARCH_TCC570X)
		if((tcc_alsa_info.cap_del_1st == false) && (tcc_alsa_info.cap_intr_byte <= 0)) {
			if((tChCtrl & Hw18) == 0) {
				tDaCdar = pADMA->RxDaCdar;
				tcc_alsa_info.cap_pos_1st = tDaCdar - pADMA->RxDaDar;
			}
			else {	//MONO
				tDaCdar = pADMA->RxDaCdarL;
				tcc_alsa_info.cap_pos_1st = tDaCdar - pADMA->RxDaDarL;
			}
			tcc_alsa_info.cap_pos_1st = (unsigned int)substream->runtime->dma_area + tcc_alsa_info.cap_pos_1st;
			alsa_dbg("cap_pos_1st=0x%08x\n", tcc_alsa_info.cap_pos_1st);
		}
#endif
#endif
	}
		...
		/* For FIFO clear */
		tca_tcc_DAI_mute(2, runtime->channels, 1);
		msleep(2);
		...
		tca_tcc_DAI_mute(2, runtime->channels, 0);
...
//I2S FIFO clear for 1st pop-noise after boot
static int first_init_delay_p = 0;
...
#if defined(CONFIG_SND_SOC_DAUDIO_FIFO_CLEAR_DELAY)
				// I2S FIFO clear for 1st pop-noise after boot
				if(!first_init_delay_sub2) {
					pDAI->DAVC = 0x10; /* Mute ON */
					ret = tcc_i2s_tx_enable(substream, 1);
					mdelay(first_init_delay_ms_sub2);
					first_init_delay_sub2 = 1;
				}
				else
					ret = tcc_i2s_tx_enable(substream, 1);
#else
...
static int tcc_pcm_close(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		//This is to remove 1st fifo.
		tcc_alsa_info.cap_del_1st = false;
		tcc_alsa_info.cap_intr_byte = 0;
...
int tcc_pcm_sub2_platform_register(struct device *dev)
{
	return snd_soc_register_platform(dev, &tcc_soc_platform);
}
EXPORT_SYMBOL_GPL(tcc_pcm_sub2_platform_register);

void tcc_pcm_sub2_platform_unregister(struct device *dev)
{
	return snd_soc_unregister_platform(dev);
}
EXPORT_SYMBOL_GPL(tcc_pcm_sub2_platform_unregister);
