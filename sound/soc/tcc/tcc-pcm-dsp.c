...
void tcc_dma_done_handler(int playback/*int irq, void *dev_id*/)
{
...
static int tcc_pcm_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
	...
	tc_dsp_setAudioBuffer(I2S0params,(unsigned long)prtd->dma_buffer,totsize,period_bytes,1);
	tc_dsp_setI2sMasterMode(I2S0params,0);
	tc_dsp_setI2sFormat(I2S0params, format /*SNDRV_PCM_FORMAT_S24_LE*/);
	tc_dsp_setI2sSamplerate(I2S0params, params_rate(params));
	tc_dsp_setI2sChannelNum(I2S0params,chs );
	tc_dsp_setPCMChannelNum(I2S0params, chs);
    } else {
	set_dma_inbuffer(prtd->dma_buffer, totsize, period_bytes,format,chs);
	tc_dsp_setAudioBuffer(I2S0_Rx_params,(unsigned long)prtd->dma_buffer,totsize,period_bytes,0);
	tc_dsp_setI2sMasterMode(I2S0_Rx_params,0);
	tc_dsp_setI2sFormat(I2S0_Rx_params, format /*SNDRV_PCM_FORMAT_S24_LE*/);
	tc_dsp_setI2sSamplerate(I2S0_Rx_params, params_rate(params));
	tc_dsp_setI2sChannelNum(I2S0_Rx_params,chs );
	tc_dsp_setPCMChannelNum(I2S0_Rx_params, chs);
    }
...
static int tcc_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
        ...
        if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
            ...
            // Start ~~~~~~~~~~~~ ?AA7s
            tc_dsp_DoPlayback(I2S0params,TCC_DSP_IPC_ID_ALSA_MAIN);
			//tcc_gpio_config(TCC_GPE(20), GPIO_FN0|GPIO_OUTPUT|GPIO_HIGH);		
        } else { // capture

        	alsa_dbg("%s() recording start\n", __func__);
            // Stop ~~~~~~~~~~~~ ?AA7s
            tc_dsp_DoPlayback(I2S0_Rx_params,TCC_DSP_IPC_ID_ALSA_IN_MAIN);
        }
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		...
		tc_dsp_DoStop(I2S0params,TCC_DSP_IPC_ID_ALSA_MAIN);
		//tcc_gpio_config(TCC_GPE(20), GPIO_FN0|GPIO_OUTPUT|GPIO_LOW);		
        } else {
		alsa_dbg("%s() recording end\n", __func__);
		tc_dsp_DoStop(I2S0_Rx_params,TCC_DSP_IPC_ID_ALSA_IN_MAIN);
        }
...
static snd_pcm_uframes_t tcc_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		tc_dsp_getCurrAudioBuffer(I2S0params, (unsigned long*)&ptr,1);
	else
		tc_dsp_getCurrAudioBuffer(I2S0_Rx_params, (unsigned long*)&ptr,0);
...
static int tcc_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		tc_dsp_setI2sNum(I2S0params,I2S0params);
	else
		tc_dsp_setI2sNum(I2S0_Rx_params,I2S0_Rx_params);
