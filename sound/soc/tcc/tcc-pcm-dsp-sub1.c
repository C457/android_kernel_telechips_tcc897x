...
void tcc_dma1_done_handler(int playback/*int irq, void *dev_id*/)
{
...
static int tcc_pcm_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
	...
	tc_dsp_setAudioBuffer(I2S1params,(unsigned long)prtd->dma_buffer,totsize,period_bytes,1);
	tc_dsp_setPCMChannelNum(I2S1params, chs);
    } else {
	set_dma_inbuffer(prtd->dma_buffer, totsize, period_bytes,format,chs);
	tc_dsp_setAudioBuffer(I2S1params,(unsigned long)prtd->dma_buffer,totsize,period_bytes,0);
	tc_dsp_setPCMChannelNum(I2S1params, chs);
    }

    tc_dsp_setI2sMasterMode(I2S1params,0);
    tc_dsp_setI2sFormat(I2S1params, format /*SNDRV_PCM_FORMAT_S24_LE*/);
    tc_dsp_setI2sSamplerate(I2S1params, params_rate(params));
    tc_dsp_setI2sChannelNum(I2S1params,chs );
...
static int tcc_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
        ...
        if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
            ...
            // Start ~~~~~~~~~~~~ ?AA7s
            tc_dsp_DoPlayback(I2S1params,TCC_DSP_IPC_ID_ALSA_SUB01);
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
        ...
        if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
            ...
            tc_dsp_DoStop(I2S1params,TCC_DSP_IPC_ID_ALSA_SUB01);
...
static snd_pcm_uframes_t tcc_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		tc_dsp_getCurrAudioBuffer(I2S1params, (unsigned long*)&ptr,1);
	else
		tc_dsp_getCurrAudioBuffer(I2S1params, (unsigned long*)&ptr,0);
...
static int tcc_pcm_open(struct snd_pcm_substream *substream)
{
    ...
    tc_dsp_setI2sNum(I2S1params,I2S1params);
...
int tcc_pcm_ch1_platform_register(struct device *dev)
{
	return snd_soc_register_platform(dev, &tcc_soc_platform);
}
EXPORT_SYMBOL_GPL(tcc_pcm_ch1_platform_register);

void tcc_pcm_ch1_platform_unregister(struct device *dev)
{
	return snd_soc_unregister_platform(dev);
}
EXPORT_SYMBOL_GPL(tcc_pcm_ch1_platform_unregister);
