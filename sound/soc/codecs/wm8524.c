...
static struct wm8524_priv *gp_wm8524=NULL;
...
static int wm8524_suspend(struct snd_soc_codec *codec)
{
	...
	printk("%s(): in...\n", __func__);
#if 0	
	pin = devm_pinctrl_get_select(codec->dev, "suspend");
	if (IS_ERR(pin))
	{
		printk("%s: get pinctrl error !! \n",__func__);
	}
#endif	
...
static int wm8524_resume(struct snd_soc_codec *codec)
{
	...
	printk("%s(): out..\n", __func__);
#if 0	
	pin = devm_pinctrl_get_select(codec->dev, "resume");
	if (IS_ERR(pin))
	{
		printk("%s: get pinctrl error !! \n",__func__);
	}
...
static int wm8524_probe(struct snd_soc_codec *codec)
{
	...
	if(gp_wm8524!=NULL)
	{
		memcpy(wm8524,gp_wm8524,sizeof(struct wm8524_priv));
		kfree(gp_wm8524);
		gp_wm8524 = NULL;
	}
...
static int wm8524_codec_probe(struct platform_device *pdev)
{
	...
	gp_wm8524 = kzalloc(sizeof(struct wm8524_priv), GFP_KERNEL);
	if (gp_wm8524 == NULL) {
		snd_soc_unregister_codec(&pdev->dev);
		return -ENOMEM;
	}
	
	np = of_find_compatible_node(NULL, NULL, "telechips,snd-wm8524");
	if (np)
	{
		codec_aif = of_get_named_gpio(np, "codec_aif-gpios", 0);
		if (gpio_is_valid(codec_aif)) {
			gp_wm8524->codec_aif = codec_aif;
		}
		
		mute_ang = of_get_named_gpio(np, "codec_mute_ang-gpios", 0);
		if (gpio_is_valid(mute_ang)) {
			gp_wm8524->mute_ang = mute_ang;
			
			if(of_property_read_u32(np, "codec_mute_ang-mute_value", &gp_wm8524->mute_value)){
				printk("%s Error get mute_value (%d) \n",__func__,gp_wm8524->mute_value);
				gp_wm8524->mute_value= 0;
			}
			gpio_set_value(mute_ang, !gp_wm8524->mute_value);
		}
		
		mute_ctl = of_get_named_gpio(np, "codec_mute_ctl-gpios", 0);
		if (gpio_is_valid(mute_ctl)) {
			gp_wm8524->mute_ctl = mute_ctl;
		}
	}
