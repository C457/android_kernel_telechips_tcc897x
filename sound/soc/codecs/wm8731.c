...
struct snd_soc_codec *codec_bak;
...
/* Telechips : ehk23 add for support kernel 3.18 WM8731 driver.*/
static const unsigned int wm8731_rates_for_telechips[] = {
...
/* Telechips : ehk23 add for support kernel 3.18 WM8731 driver.*/
static const
struct snd_pcm_hw_constraint_list wm8731_constraints_for_telechips = {
	.list = wm8731_rates_for_telechips,
	.count = ARRAY_SIZE(wm8731_rates_for_telechips),
};
		...
		int clk_id, unsigned int freq, int dir)
{
	...
	switch (freq) {
		case 0:
			...
			break;
#if defined(CONFIG_ARCH_TCC) //ehk23
		case 12000000:
		case 12288000:
		case 18432000:
		case 16934400:
		case 11289600:
			wm8731->constraints = &wm8731_constraints_for_telechips;
			break;
#else
		case 12000000:
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		...
		reg = snd_soc_read(codec, WM8731_PWR) & 0x7f;	// Planet 20130904  0xff7f->0x7f
		snd_soc_write(codec, WM8731_PWR, reg | 0x40);	// Planet 20130904  0x0040->0x40
		// Planet 20130904 Start
		for(i=0; i<10; i++) {
			read = snd_soc_read(codec, i);
			snd_soc_write(codec, i, read);
		}
		// Planet 20130904 End
		break;
	case SND_SOC_BIAS_OFF:
		snd_soc_write(codec, WM8731_PWR, 0xff);		//  Planet 20130904  0xffff->0xff
...
static int wm8731_suspend_dt(struct i2c_client *client)
{
	wm8731_set_bias_level(codec_bak, SND_SOC_BIAS_OFF);

	return 0;
}

static int wm8731_resume_dt(struct i2c_client *client)
{
	wm8731_set_bias_level(codec_bak, SND_SOC_BIAS_STANDBY);

	return 0;
}

static const struct dev_pm_ops tcc_wm8731_pm_ops = {
	.freeze    = wm8731_suspend_dt,
	.thaw      = wm8731_resume_dt,
	.restore   = wm8731_resume_dt,
}
