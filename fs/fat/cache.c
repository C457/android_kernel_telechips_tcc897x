...
/* readdir2 porting -> start */
extern int fat_ent_read_raw(struct super_block *, struct fat_entry *, int);

int fat_get_cluster_nocache(struct super_block *sb, sector_t i_start,
		sector_t i_pos, int cluster, int *fclus, int *dclus)
{
	const int limit = sb->s_maxbytes >> MSDOS_SB(sb)->cluster_bits;
	struct fat_entry fatent;
	int nr;
	...
	*dclus = i_start;
	...
	while (*fclus < cluster) {
		...
		nr = fat_ent_read_raw(sb, &fatent, *dclus);
		if (nr < 0)
			goto out;
		else if (nr == FAT_ENT_FREE) {
...
/* readdir2 porting -> start */
static int fat_bmap_cluster_nocache(struct super_block *sb, sector_t i_start,
		sector_t i_pos, int cluster)
{
	int ret, fclus, dclus;
	...
	ret = fat_get_cluster_nocache(sb, i_start, i_pos, cluster, &fclus, &dclus);
	if (ret < 0)
		return ret;
	else if (ret == FAT_ENT_EOF) {
		fat_fs_error(sb, "%s: request beyond EOF (i_pos %lld)",
...
/* readdir2 porting -> start */
int fat_bmap_raw(struct super_block *sb, sector_t i_start, sector_t i_pos,
		sector_t sector, sector_t *phys)
{
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
	int cluster, offset;
	...
	*phys = 0;
	...
	cluster = sector >> (sbi->cluster_bits - sb->s_blocksize_bits);
	offset  = sector & (sbi->sec_per_clus - 1);
	cluster = fat_bmap_cluster_nocache(sb, i_start, i_pos, cluster);
	if (cluster < 0)
		return cluster;
	else if (cluster)
		*phys = fat_clus_to_blknr(sbi, cluster) + offset;
