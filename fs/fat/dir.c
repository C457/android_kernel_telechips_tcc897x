...
extern int is_exec(unsigned char *extension);
static int __fat_dir_dot_and_dotdot_exists_raw(sector_t, sector_t, struct super_block *);
...
/* readdir2 porting -> start */
extern int fat_bmap_raw(struct super_block *, sector_t, sector_t, sector_t, sector_t *);

/* will be used for dot and dotdot only */
static int fat__get_entry_dots(struct super_block *sb,
			  sector_t i_start, sector_t i_pos,
			  loff_t *pos,
			  struct buffer_head **bh, struct msdos_dir_entry **de)
{
	sector_t phys, iblock;
	...
	*bh = NULL;
	iblock = *pos >> sb->s_blocksize_bits;
	err = fat_bmap_raw(sb, i_start, i_pos, iblock, &phys);
	if (err || !phys)
		return -1;	/* beyond EOF or error */
...
/* readdir2 porting -> start */
/* will be used for dot and dotdot only */
static inline int fat_get_entry_dots(struct super_block *sb,
				sector_t i_start, sector_t i_pos,
				loff_t *pos,
				struct buffer_head **bh,
				struct msdos_dir_entry **de)
{
	/* Fast stuff first */
	if (*bh && *de &&
	    (*de - (struct msdos_dir_entry *)(*bh)->b_data) < MSDOS_SB(sb)->dir_per_block - 1) {
		*pos += sizeof(struct msdos_dir_entry);
		(*de)++;
		return 0;
	}
	return fat__get_entry_dots(sb, i_start, i_pos, pos, bh, de);
}
/* readdir2 porting -> end */
...
/* readdir2 porting -> start */
static int __fat_readdir_avn(struct inode *inode, struct file *file,
			 struct dir_context_avn *ctx, int short_only,
			 struct fat_ioctl_filldir_callback *both)
{
	struct super_block *sb = inode->i_sb;
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
...
start_filldir:
	...
	if (!memcmp(de->name, MSDOS_DOT, MSDOS_NAME)) {
        ...
        fat_time_fat2unix(sbi, &mtime_ts, de->time, de->date, 0);
        mtime = mtime_ts.tv_sec;
        filesize = de->size;
        if ((de->attr & ATTR_DIR) && !IS_FREE(de->name))    // it's directory
            mode = fat_make_mode(sbi, de->attr, S_IRWXUGO);
        else
            mode = fat_make_mode(sbi, de->attr, ((sbi->options.showexec && 
                !is_exec(de->name + 8)) ? S_IRUGO|S_IWUGO : S_IRWXUGO));

        if ((de->attr & ATTR_DIR) && !IS_FREE(de->name)) {
            int i_ret;
            sector_t i_start = le16_to_cpu(de->start);
            if (sbi->fat_bits == 32)
                i_start |= (le16_to_cpu(de->starthi) << 16);
            i_ret = __fat_dir_dot_and_dotdot_exists_raw(i_start, i_pos, sb);
            if (!i_ret) {
                char pathnamebuf[64] = {0,};
                struct fat_ioctl_filldir_callback *p = ctx->actor;
                const char *de_name;
                int de_name_len = 0;
                if (fill_name) {
                    de_name = fill_name;
                    de_name_len = fill_len;
                } else {
                    de_name = p->longname;
                    de_name_len = p->long_len;
                }
                if (de_name_len > sizeof(pathnamebuf))
		...
		if (!dir_emit_avn(ctx, fill_name, fill_len, inum,
			    (de->attr & ATTR_DIR) ? DT_DIR : DT_REG, mode, mtime, filesize))
			goto fill_failed;
	}
...
static int fat_readdir_avn(struct file *file, struct dir_context_avn *ctx)
{
	return __fat_readdir_avn(file_inode(file), file, ctx, 0, NULL);
}
/* readdir2 porting -> end */
...
/* readdir2 porting -> start */
/* will be used for dot and dotdot only */
static int fat_get_short_entry_dots(struct super_block *sb,
			       sector_t i_start, sector_t i_pos,
			       loff_t *pos,
			       struct buffer_head **bh,
			       struct msdos_dir_entry **de)
{
	int ret = fat_get_entry_dots(sb, i_start, i_pos, pos, bh, de);
	if (ret < 0)
		return -ENOENT;
	return 0;
}
...
/* readdir2 porting -> start */
static noinline int __fat_dir_dot_and_dotdot_exists_raw(
		sector_t i_start, sector_t i_pos,
		struct super_block *sb)
{
	...
	/* The first and second entries must be '.' + 0x20's
	 * and '..' + 0x20's on FAT fs.
	 */
	while (count++ < 2 &&
			fat_get_short_entry_dots(sb, i_start, i_pos, &cpos, &bh, &de) >= 0) {
		if ((result == 0 &&
