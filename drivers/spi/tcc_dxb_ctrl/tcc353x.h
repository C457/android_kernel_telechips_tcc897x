...
static inline int tcc353x_init(struct tcc_dxb_ctrl_t *ctrl, int deviceIdx)
{
...
static inline int tcc353x_reset(struct tcc_dxb_ctrl_t *ctrl, int deviceIdx)
{
	switch(deviceIdx)
	{
	case 0:
...
static inline int tcc353x_on(struct tcc_dxb_ctrl_t *ctrl, int deviceIdx)
{
	...
	if(log_on){
		GPIO_SET_VALUE(ctrl->gpio_ant_pwr, 1);  //ANT POWER ON
		log_on = false;
	}
...
static inline int tcc353x_off(struct tcc_dxb_ctrl_t *ctrl, int deviceIdx)
{
	if (deviceIdx >= MAX_DEVICE_NO)
		return 0;
	...
	case 0:
		...
		GPIO_SET_VALUE(ctrl->gpio_ant_pwr, 0);  //ANT POWER OFF
		break;
...
static inline int TCC353X_IOCTL(struct tcc_dxb_ctrl_t *ctrl, unsigned int cmd, unsigned long arg)
{
