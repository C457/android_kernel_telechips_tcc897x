...
static inline int tcc3161_init(struct tcc_dxb_ctrl_t *ctrl, int deviceIdx)
{
...
static inline int tcc3161_reset(struct tcc_dxb_ctrl_t *ctrl, int deviceIdx)
{
	switch(deviceIdx)
	{
	case 0:
...
static inline int tcc3161_on(struct tcc_dxb_ctrl_t *ctrl, int deviceIdx)
{
...
static inline int tcc3161_off(struct tcc_dxb_ctrl_t *ctrl, int deviceIdx)
{
	if (deviceIdx >= MAX_DEVICE_NO)
		return 0;
...
static inline int TCC3161_IOCTL(struct tcc_dxb_ctrl_t *ctrl, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		case IOCTL_DXB_CTRL_SET_BOARD:
			...
			tcc3161_init(ctrl, deviceIdx);
			break;

		case IOCTL_DXB_CTRL_OFF:
			deviceIdx = (arg == 0) ? 0 : *(unsigned int *)arg;
			tcc3161_off(ctrl, deviceIdx);
			break;

		case IOCTL_DXB_CTRL_ON:
			deviceIdx = (arg == 0) ? 0 : *(unsigned int *)arg;
			tcc3161_on(ctrl, deviceIdx);
			break;

		case IOCTL_DXB_CTRL_RESET:
			deviceIdx = (arg == 0) ? 0 : *(unsigned int *)arg;
			tcc3161_reset(ctrl, deviceIdx);
			break;
