...
#endif
#define SUPPORTED_TSIF TSIF_MODE_HWDMX
 ...
 * External Functions
 ******************************************************************************/
extern int tcc_tsif_buffer_flush_ex(unsigned int tsif_type, struct file *filp, unsigned long p, int size);
extern int tcc_tsif_set_external_tsdemux_ex(unsigned int tsif_type, struct file *filp, int (*decoder)(char *p1, int p1_size, char *p2, int p2_size, int devid), int max_dec_packet, int devid);
extern int tcc_tsif_open_ex(unsigned int tsif_type, struct inode *inode, struct file *filp);
extern int tcc_tsif_release_ex(unsigned int tsif_type, struct inode *inode, struct file *filp);
extern int tcc_tsif_ioctl_ex(unsigned int tsif_type, struct file *filp, unsigned int cmd, unsigned long arg, int idevid);
...
static void tcc_tsif_running_feed(long unsigned int id)
{
	...
	for(index=0; index<TS_PACKET_LIST; index++)
	{
		...
		if (type == TSIF_FILTER_TYPE_SECTION)
		{
			if (p2_size > 0)
			{
				tcc_tsif_buffer_flush_ex(SUPPORTED_TSIF, &pHandle->filp, (unsigned long)p2, p2_size);
			}
		...
		else
		{
			if (p1_size > 0)
			{
				tcc_tsif_buffer_flush_ex(SUPPORTED_TSIF, &pHandle->filp, (unsigned long)p1, p1_size);
			}
			if (p2_size > 0)
			{
				tcc_tsif_buffer_flush_ex(SUPPORTED_TSIF, &pHandle->filp, (unsigned long)p2, p2_size);
...
#else
		if (tcc_dmx_ts_callback(p1, p1_size, p2, p2_size, devid) != 0)
		{
			tcc_tsif_ioctl_ex(SUPPORTED_TSIF,0, IOCTL_TSIF_RESET, 0, 0);
		}
#endif
	}
...
int tcc_tsif_interface_open(unsigned int devid)
{
	...
	if (pHandle->users == 1)
	{
		...
		err = tcc_tsif_open_ex(SUPPORTED_TSIF, &inode, &pHandle->filp);
		if (err != 0)
		{
			pHandle->users = 0;
		...
		tcc_tsif_set_external_tsdemux_ex(SUPPORTED_TSIF, &pHandle->filp, tcc_tsif_parse_packet, 1, 0);
	}
...
int tcc_tsif_interface_close(unsigned int devid)
{
	...
	if (pHandle->users == 0)
	{
		tcc_tsif_set_external_tsdemux_ex(SUPPORTED_TSIF, &pHandle->filp, tcc_tsif_parse_packet, 0, 0);
		for (type = 0; type < TSIF_FILTER_TYPE_MAX; type++)
		{
			tasklet_kill(&pHandle->task[type].tsif_tasklet);
		}
		inode.i_rdev = MKDEV(0, devid);
		err = tcc_tsif_release_ex(SUPPORTED_TSIF, &inode, &pHandle->filp);
		if (err != 0)
		{
			pHandle->users = 1;
			mutex_unlock(&pHandle->mutex);
			...
			if (i == MAX_TS_CNT)
			{
				...
				for (i = 0; i < MAX_SEC_CNT; i++) // if section filter for same pid, it must be stop.
				{
						...
						pHandle->sec_table.iUseSW[i] == 0)
					{
						...
						param.f_size = 0;
						if (tcc_tsif_ioctl_ex(SUPPORTED_TSIF, &pHandle->filp, IOCTL_TSIF_REMOVE_PID, (unsigned long)&param, 0) != 0)
						{
							eprintk("%s : pid setting error !!!\n", __func__);
						}
					}
				}
#endif
				...
				param.f_pid = feed->pid;
				param.f_size = 0;
				if (tcc_tsif_ioctl_ex(SUPPORTED_TSIF, &pHandle->filp, IOCTL_TSIF_ADD_PID, (unsigned long)&param, 0) != 0)
				{
					eprintk("%s : pid setting error !!!\n", __func__);
					return -1;
				}
...
static int tcc_tsif_set_pid(tcc_tsif_priv_t *pHandle, struct dvb_demux_feed *feed)
{
	...
	if (tcc_tsif_ioctl_ex(SUPPORTED_TSIF, &pHandle->filp, IOCTL_TSIF_SET_PID, (unsigned long)&pid_param, 0) != 0)
	{
		eprintk("%s : pid setting error !!!\n", __func__);
		return -1;
	}
	if (pHandle->is_start_dma == 0)
	{
		...
		//dma_param.mode = 0x01;
		//dma_param.mode = 0x00;
		if (tcc_tsif_ioctl_ex(SUPPORTED_TSIF, &pHandle->filp, IOCTL_TSIF_DMA_START, (unsigned long)&dma_param, 0) != 0)
		{
			eprintk("%s : dma setting error !!!\n", __func__);
			return -1;
		}
	}
...
int tcc_tsif_interface_set_pcrpid(int on, struct dvb_demux_feed *feed, unsigned int devid)
{
	...
	if (on)
	{
		...
		//tcc_tsif_set_external_tsdemux_ex(SUPPORTED_TSIF, &pHandle->filp, tcc_tsif_parse_packet, 1, 0); //255->1 changed, because it controlled by cm3
#else
		tcc_tsif_interface_set_pid(feed, devid);
		tcc_tsif_set_external_tsdemux_ex(SUPPORTED_TSIF, 0, tcc_tsif_parse_packet, 1/*255*/, 0);
#endif
	}
	else
	{
...
#else
		tcc_tsif_interface_remove_pid(feed, devid);
		tcc_tsif_set_external_tsdemux_ex(SUPPORTED_TSIF, 0, tcc_tsif_parse_packet, 1, 0);
#endif
	}

	mutex_lock(&pHandle->mutex);
	ret = tcc_tsif_ioctl_ex(SUPPORTED_TSIF, &pHandle->filp, IOCTL_TSIF_SET_PCRPID, (unsigned long)&param, 0);
	mutex_unlock(&pHandle->mutex);
	...
	switch (feed->type)
	{
		case DMX_TYPE_TS:
		{
			...
			if (pHandle->ts_table.iUsers[i] == 0)
			{
				...
				param.f_pid = feed->pid;
				param.f_size = 0;
				if (tcc_tsif_ioctl_ex(SUPPORTED_TSIF, &pHandle->filp, IOCTL_TSIF_REMOVE_PID, (unsigned long)&param, 0) != 0 && param.f_pid != 0x2000)
				{
					eprintk("%s : pid setting error !!!\n", __func__);
				}
#ifdef SECFILTER_TO_TSFILTER
...
static int tcc_tsif_remove_pid(tcc_tsif_priv_t *pHandle, struct dvb_demux_feed *feed)
{
	...
	if (pHandle->is_start_dma == 0)
	{
		if(tcc_tsif_ioctl_ex(SUPPORTED_TSIF, &pHandle->filp, IOCTL_TSIF_DMA_STOP, 0, 0)!=0)
		{
			eprintk("%s : dma setting error !!!\n", __func__);
			return -1;
		}
...
unsigned int tcc_tsif_interface_get_stc(unsigned int index, unsigned int devid)
{
	...
	mutex_lock(&pHandle->mutex);
	tcc_tsif_ioctl_ex(SUPPORTED_TSIF, &pHandle->filp, IOCTL_TSIF_GET_STC, (unsigned long)&stc, 0);
	mutex_unlock(&pHandle->mutex);
