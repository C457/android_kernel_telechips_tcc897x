            ...
            if (dmxfilter->state == DMX_STATE_GO)
            {
                ...
                if (feed->type == DMX_TYPE_SEC && feed->state == DMX_STATE_GO)
                {
                    int section_syntax_indicator=0;
                     ...
                     {
                         //crc_err : 0(no crc error), 1(crc_error), 2(hwdmux cannot decide crc_error)
                         //Should check crc error, because hwdemux didn't
                         //check crc result.
...
#ifdef      TS_SECTION_CHK_MODE
                              hexdump(p, 64);
                              eprintk("[check section] section crc checking error !.section size[%d]crc_result[%d]\n", size, crc_err);
#endif                              
                          }
                     }
                    ...
                    if (!feed->feed.sec.check_crc || !crc_err)
                    {
                        ...
                        if(secsize != size)
                        {
                            eprintk("[check section] section size mismatch!-[%d][%d]\n", secsize, size);
                            hexdump(p, 64);
                        ...
                        if(feed->feed.sec.check_crc && section_syntax_indicator)
                        {
                            ...
                            if( demux->check_crc32(feed, p, size) )
                            {
                                eprintk("[check section] section crc checking error !.section size[%d]\n", size);
                                eprintk("[check section] filter values [0x%X][0x%X][0x%X][0x%X][0x%X][0x%X].\n",  secfilter->filter_value[0], secfilter->filter_value[1], secfilter->filter_value[2], \
                                        secfilter->filter_value[3], secfilter->filter_value[4], secfilter->filter_value[5]);
                                hexdump(p, 64);                          
                                hexdump(p+size-64, 64);
                            }
                        }
                        ...
                        if(has_error)
                        {
                            eprintk("[check section] section matching error!.section size[%d] crc[%d] doneq[%d]\n",size, feed->feed.sec.check_crc, dmxfilter->doneq);
                            eprintk("[check section] filter values [0x%X][0x%X][0x%X][0x%X][0x%X][0x%X].\n",  secfilter->filter_value[0], secfilter->filter_value[1], secfilter->filter_value[2], \
                                        secfilter->filter_value[3], secfilter->filter_value[4], secfilter->filter_value[5]); 
                            hexdump(p, 64);
                        }
...
static int tcc_dmx_start_feed(struct dvb_demux_feed *feed)
{
    ...
    if (tcc_tsif_interface_open(devid) == 0)
	{
#ifdef TS_PACKET_CHK_MODE
    ...
    if (feed->pid != MAX_PID)
    {
        if((feed->type == DMX_TYPE_TS) && (feed->pes_type == DMX_PES_PCR0 ||
...
static int tcc_dmx_stop_feed(struct dvb_demux_feed *feed)
{
    ...
    if (tcc_tsif_interface_close(devid) == 0)
	{
#ifdef TS_PACKET_CHK_MODE
        ts_packet_chk_t* tmp = NULL;
        ts_packet_chk_t* tmp_ = NULL;
...
int tcc_dmx_init(tcc_dmx_inst_t *inst)
{
	...
	for (i = 0; i < inst->dev_num; i++)
	{
	    ...
	    inst->dmx[i].demux.dmx.capabilities = DMX_TS_FILTERING | DMX_SECTION_FILTERING | DMX_MEMORY_BASED_FILTERING;
	    ...
	    for (j = 0; j < 4; j++)
	    {
        	...
        	if (inst->dmx[i].demux.dmx.add_frontend(&inst->dmx[i].demux.dmx, &inst->dmx[i].fe_hw[j]) < 0)
        	{
    	...
    	if (inst->dmx[i].demux.dmx.add_frontend(&inst->dmx[i].demux.dmx, &inst->dmx[i].fe_mem) < 0)
    	{
	        eprintk("%s(fail - add_frontend fe_mem)\n", __FUNCTION__);
        	return -1;
    	}

	    if (inst->dmx[i].demux.dmx.connect_frontend(&inst->dmx[i].demux.dmx, &inst->dmx[i].fe_hw[0]) < 0)
	    {
