...
int hmgr_list_manager(void* args, unsigned int cmd);
/////////////////////////////////////////////////////////////////////////////
...
{	
	...
	if(!hmgr_get_close(type))
	{
		...
		hmgr_list_manager(( void *)cmd_list, LIST_ADD);
...
static int _hmgr_process(vputype type, int cmd, int pHandle, void* args)
{
	int ret = 0;
#ifdef CONFIG_VPU_TIME_MEASUREMENT
		...
		switch(cmd)
		{
			case VPU_DEC_INIT:
				{
					...
					arg->gsHevcDecInit.m_RegBaseVirtualAddr = io_p2v(hmgr_data.base_addr);
					arg->gsHevcDecInit.m_Memcpy 			= (void* (*) ( void*, const void*, unsigned int ))memcpy;
					arg->gsHevcDecInit.m_Memset 			= (void* (*) ( void*, int, unsigned int ))memset;
					arg->gsHevcDecInit.m_Interrupt			= (int	(*) ( void ))_hmgr_internal_handler;

					hmgr_data.check_interrupt_detection = 1;
					dprintk("@@ Dec :: Init In => workbuff 0x%x/0x%x, Reg: 0x%x/0x%x, format : %d, Stream(0x%x/0x%x, 0x%x)\n",
			...
			case VPU_DEC_SEQ_HEADER:
				{
					...
					ret = tcc_hevc_dec(cmd, &pHandle, (void*)iSize, (void*)(&arg->gsHevcDecInitialInfo));
			...
			case VPU_DEC_REG_FRAME_BUFFER:
				{
					...
					ret = tcc_hevc_dec(cmd, &pHandle, (void*)(&arg->gsHevcDecBuffer), (void*)NULL);
			...
			case VPU_DEC_DECODE:
				{						
					...
					hmgr_data.check_interrupt_detection = 1;
					ret = tcc_hevc_dec(cmd, &pHandle, (void*)(&arg->gsHevcDecInput), (void*)(&arg->gsHevcDecOutput));
			...
			case VPU_DEC_BUF_FLAG_CLEAR:
				{
					...
					ret = tcc_hevc_dec(cmd, &pHandle, (void*)(arg), (void*)NULL);
				}
				break;
			...
			case VPU_DEC_CLOSE:
				{
					...
					arg = (HEVC_DECODE_t *)args;
					hmgr_data.check_interrupt_detection = 1;
					ret = tcc_hevc_dec(cmd, &pHandle, (void*)NULL, (void*)NULL/*(&arg->gsHevcDecOutput)*/);					
			...
			case GET_RING_BUFFER_STATUS:
				{
					...
					ret = tcc_hevc_dec(cmd, &pHandle, (void*)NULL, (void*)(&arg->gsHevcDecRingStatus));
				}
				break;
			case FILL_RING_BUFFER_AUTO:
				{
					...
					hmgr_data.check_interrupt_detection = 1;
					ret = tcc_hevc_dec(cmd, &pHandle, (void*)(&arg->gsHevcDecInit), (void*)(&arg->gsHevcDecRingFeed));
					dprintk("@@ Dec :: ReadPTR : 0x%08x, WritePTR : 0x%08x\n", *(volatile unsigned int *)(io_p2v(hmgr_data.base_addr) + 0x120), *(volatile unsigned int *)(io_p2v(hmgr_data.base_addr) + 0x124));
				}
				break;
			...
			case VPU_UPDATE_WRITE_BUFFER_PTR:
				{
					...
					hmgr_data.check_interrupt_detection = 1;
					ret = tcc_hevc_dec(cmd, &pHandle, (void*)(arg->iCopiedSize), (void*)(arg->iFlushBuf));
				}
				break;
			...
			case GET_INITIAL_INFO_FOR_STREAMING_MODE_ONLY:
				{
					...
					arg = (HEVC_SEQ_HEADER_t *)args;
					hmgr_data.check_interrupt_detection = 1;
					ret = tcc_hevc_dec(cmd, &pHandle, (void*)(&arg->gsHevcDecInitialInfo), NULL);
				}
				break;
			...
			case VPU_CODEC_GET_VERSION:
				{
					...
					hmgr_data.check_interrupt_detection = 1;
					ret = tcc_hevc_dec(cmd, &pHandle, arg->pszVersion, arg->pszBuildData);
			...
			case VPU_DEC_SWRESET:
				{
					ret = tcc_hevc_dec(cmd, &pHandle, NULL, NULL);
				}
				break;
		...
		case VPU_SET_CLK:
			if(copy_from_user(&info,(CONTENTS_INFO*)arg,sizeof(info)))
	...
	#if defined(CONFIG_CLOCK_TABLE)
				{
					...
					clk_idx = hmgr_calcClock(info);								
					ret = hmgr_setClock(clk_idx, info, hmgr_data.dev_opened, hmgr_data.clk_limitation);
				}
	#endif
			}
			break;
...
static int _hmgr_proc_exit_by_external(struct VpuList *list, int *result, unsigned int type)
{
	if(!hmgr_get_close(type) && hmgr_data.handle[type] != 0x00)
	{
		...
		hmgr_list_manager(( void *)list, LIST_ADD);
...
static int _hmgr_open(struct inode *inode, struct file *filp)
{
	...
	if(hmgr_data.dev_opened == 0)
	{
		hmgr_readyClock(hmgr_data.dev_opened, hmgr_data.base_addr);
	...
	hmgr_log_clkstatus(0, hmgr_data.dev_opened, hmgr_data.nOpened_Count);
...
//////////////////////////////////////
	...
	hmgr_disable_clock();
	hmgr_clearClock(hmgr_data.dev_opened);

	hmgr_data.nOpened_Count++;
	hmgr_log_clkstatus(1, hmgr_data.dev_opened, hmgr_data.nOpened_Count);
...
int hmgr_list_manager(void* args, unsigned int cmd)
{
	...
	*/
		switch(cmd)
		{
			...
			case LIST_IS_EMPTY:
				ret = list_empty(&hmgr_data.comm_data.main_list);
				break;
			...
			case LIST_GET_ENTRY:
				...
				*data_address = (unsigned int)list_first_entry(&hmgr_data.comm_data.main_list, VpuList_t, list);
				break;
		}
	}
...
static int _hmgr_operation(void)
{
	...
	while(!hmgr_list_manager(NULL, LIST_IS_EMPTY))
	{
		...
		hmgr_list_manager(&data, LIST_GET_ENTRY);
		if(!data) {
			err("data is null \n");
			hmgr_data.cmd_processing = 0;
			return 0;
		...
		*(oper_data->vpu_result) |= RET2;        
        ...
        hmgr_list_manager((void*)oper_data, LIST_DEL);
...
int hmgr_probe(struct platform_device *pdev)
{
	...
	dprintk("============> HEVC base address [0x%x], irq num [%d] \n", hmgr_data.base_addr, hmgr_data.irq - 32);
...
//	spin_lock_init(&(hmgr_data.comm_data.lock));
	...
	kidle_task = kthread_run(_hmgr_thread, NULL, "khmgr_operd");	
...
int hmgr_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if(hmgr_data.dev_opened != 0)
	{
		printk(" \n hevc: suspend In DEC(%d/%d/%d/%d) \n", hmgr_get_close(VPU_DEC), hmgr_get_close(VPU_DEC_EXT), hmgr_get_close(VPU_DEC_EXT2), hmgr_get_close(VPU_DEC_EXT3));
		...
		printk("hevc: suspend Out DEC(%d/%d/%d/%d) \n\n", hmgr_get_close(VPU_DEC), hmgr_get_close(VPU_DEC_EXT), hmgr_get_close(VPU_DEC_EXT2), hmgr_get_close(VPU_DEC_EXT3));
	}
