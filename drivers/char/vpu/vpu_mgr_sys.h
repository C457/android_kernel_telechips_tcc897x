...
extern int vmgr_calcClock(CONTENTS_INFO info);
extern int vmgr_setClock(int in_idx, CONTENTS_INFO info, unsigned char dev_opened_cnt, unsigned char clk_limitation);
#endif

extern void vmgr_readyClock(unsigned char count, unsigned int vpu_base);
extern void vmgr_clearClock(unsigned char count);

extern void vmgr_set_clkstatus(vputype type, int on);
extern void vmgr_log_clkstatus(unsigned char closed, unsigned char opened_count, unsigned int accumulated_count);
...
extern int  vmgr_request_irq(unsigned int irq, irqreturn_t (*handler)(int , void *), unsigned long frags, const char * device, void * dev_id);
