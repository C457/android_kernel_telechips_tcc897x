...
int fc_video_config(struct hdmi_tx_dev *dev, videoParams_t *video)
{
	...
	if((dev == NULL) || (video == NULL)){
		LOGGER(SNPS_ERROR, "Invalid arguments: dev=%x; video=%x", (unsigned int)dev, (unsigned int)video);
		return FALSE;
	}
	...
	if (video->mHdmiVideoFormat == 2) {
		if (video->mHdmiVideoFormat == 0) {	/* 3d data frame packing is transmitted as a progressive format */
