...
void tccxx_jpgdec_reset_clock(unsigned char deinit)
{
	...
	if(deinit)
	{
		clk_disable_unprepare(jpegd_clk);
	}
	else
	{
		clk_disable_unprepare(vcore_clk);			
...
static int tcc_jpegdec_probe(struct platform_device *pdev)
{
	
	printk("_______tcc_jpeg_dec_probe start!!! \n");
	
	if(pdev->dev.of_node)
	{
#ifndef CONFIG_SUPPORT_TCC_JPU	
		vcore_clk = of_clk_get(pdev->dev.of_node, 0);
		jpegd_clk = of_clk_get(pdev->dev.of_node, 1);
#endif
	}
	else
	{
...
static int tcc_jpegdec_remove(struct platform_device *pdev)
{
	return 0;	
}


static struct of_device_id jpegdec_of_match[] = {
	{ .compatible = "telechips-jpegdec" },
	{}
};
MODULE_DEVICE_TABLE(of, jpegdec_of_match);

static struct platform_driver tcc_jpegdec_driver = {
	.driver = {
		.name 	= TCC_JPEGDEC_DEVICE_NAME,
		.owner 	= THIS_MODULE,
		.of_match_table = of_match_ptr(jpegdec_of_match),
	},
	.probe		= tcc_jpegdec_probe,
	.remove		= tcc_jpegdec_remove
};
...
{
	platform_driver_unregister(&tcc_jpegdec_driver);
...
tcc_jpegdec_init(void)
{
	...
	platform_driver_register(&tcc_jpegdec_driver);	
