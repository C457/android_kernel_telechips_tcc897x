...
void tcc_hdmi_audio_set_clock(unsigned int output, unsigned int clock_rate)
{
    ...
    else if( output == SPDIF_PORT )
    {
		...
		if(spdif_pclk != NULL)
		{
			clk_set_rate(spdif_pclk, clk_rate);
		}
...
static int audio_probe(struct platform_device *pdev)
{
	...
	if (hdmi_audio_pclk == NULL && hdmi_audio_hclk == NULL ) {
		...
		spdif_node = of_parse_phandle(pdev->dev.of_node,"spdif-block", 0);
		if (spdif_node) 
		{
			spdif_pclk = of_clk_get(spdif_node, 0);
			if (IS_ERR(spdif_pclk)){
				printk(KERN_WARNING "HDMI: failed to get spdif_pclk \n");
				spdif_pclk  = NULL;
			}
		}
	}
...
static struct of_device_id audio_of_match[] = {
	...
	{ .compatible = "telechips,tcc896x-hdmi-audio" },
