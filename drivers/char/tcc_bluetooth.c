...
static int wifi_reg_on_port	= (int)NULL;
#ifdef CONFIG_DAUDIO_KK
...
#define BT_RESET_MAX 10
/* rfkill_ops callback. Turn transmitter on when blocked is false */
static int tcc_bt_rfkill_set_power(void *data, bool blocked)
{
    printk("[## BT ##] tcc_bt_rfkill_set_power BLOCKED[%d]\n", blocked);
    int retry_cnt = 0;

    if (!blocked) {
        if (gpio_is_valid(bt_reg_on_port))
            gpio_set_value_cansleep(bt_reg_on_port, 0);     /* reg on */
        if (gpio_is_valid(bt_wake_port))
            gpio_direction_output(bt_wake_port, 1);         /* wake */
        if (gpio_is_valid(bt_hwake_port))
            gpio_direction_input(bt_hwake_port);            /* host wake */
        if (gpio_is_valid(wifi_reg_on_port))
            gpio_set_value_cansleep(wifi_reg_on_port,1);    /* wifi reg on */
        if (gpio_is_valid(bt_reg_on_port))
            gpio_set_value_cansleep(bt_reg_on_port, 1);     /* reg on */
#ifdef CONFIG_DAUDIO_KK
        if (gpio_is_valid(bt_reset_port)){
            gpio_set_value_cansleep(bt_reset_port, 0);     /* bt reset */
	    msleep(30);
            gpio_set_value_cansleep(bt_reset_port, 1);     /* bt reset */
	    do{
		    if(gpio_get_value(bt_reset_port))
		    {
    	    		printk("[## BT ##] %s : SET_BT_ON[%d]RETRY[%d]\n", __func__, gpio_get_value(bt_reset_port), retry_cnt);
			break;
		    }else{
            		gpio_set_value_cansleep(bt_reset_port, 1);     /* bt reset */
			    retry_cnt++;
		    }
	    }while(retry_cnt < BT_RESET_MAX);
	}
	if(retry_cnt == 10)
    	    printk("[## BT ##] %s : SET_BT_FAIL[%d]RETRY[%d]\n", __func__, gpio_get_value(bt_reset_port), retry_cnt);
        ...
        if (gpio_is_valid(bt_reset_port)){
            gpio_set_value_cansleep(bt_reset_port, 0);     /* bt reset */
    	    printk("[## BT ##] %s : SET_BT_OFF[%d]\n", __func__, gpio_get_value(bt_reset_port));
	}
#endif
    }
...
static int tcc_bluetooth_probe(struct platform_device *pdev)
{
    ...
    if (np) {
        ...
        if (gpio_is_valid(bt_reg_on_port)) {
            ...
            gpio_direction_output(bt_reg_on_port, 1);
        } else {
            printk("[## BT ##] %s: err to get gpios: ret:%x\n", __func__, bt_reg_on_port);
            bt_reg_on_port = -1;
        }

        bt_wake_port = of_get_named_gpio(np, "bt_wake-gpios", 0);
        if (gpio_is_valid(bt_wake_port)) {
...
#endif
#ifndef CONFIG_DAUDIO_KK
        wifi_reg_on_port = of_get_named_gpio(np, "wifi_reg_on-gpio", 0);
        if (gpio_is_valid(wifi_reg_on_port)) {
            gpio_request(wifi_reg_on_port, "wifi_reg_on");
            gpio_set_value_cansleep(wifi_reg_on_port,1);
        } else {
            printk("[## BT ##] %s: err to get gpios: ret:%x\n", __func__, wifi_reg_on_port);
            wifi_reg_on_port = -1;
        }
#endif
#ifdef CONFIG_DAUDIO_KK
	bt_reset_port = of_get_named_gpio(np, "bt_reset-gpio", 0);
        if (gpio_is_valid(bt_reset_port)) {
            gpio_request(bt_reset_port, "bt_reset");
            gpio_set_value_cansleep(bt_reset_port,1);
            printk("[## BT ##] %s: BT_RESET[%d], WIFI_REG_ON[%d]", __func__, gpio_get_value(bt_reset_port), gpio_get_value(wifi_reg_on_port));
        } else {
