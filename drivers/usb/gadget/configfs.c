...
static unsigned int android_index;
...
static void android_work(struct work_struct *data)
{
	...
	if (status[0]) {
		kobject_uevent_env(&gi->android_device->kobj,
...
int get_free_device_index(void)
{
	int i;

	for(i=0;i<32;i++)
		if(!(android_index&(1<<i))){
			BITSET(android_index, (1<<i));
			return i;
		}
...
void clear_device_index(unsigned int index)
{
	if(!(android_index & (1<<index)))
		printk("%d bit already been set.\n", index);

	BITCLR(android_index, (1<<index));
}
...
static int android_device_create(struct gadget_info *gi)
{
	...
	char *device_name[16];
	u32 device_index = get_free_device_index();

	INIT_WORK(&gi->work, android_work);
	sprintf(device_name, "android%d",device_index);
	gi->android_device = device_create(android_class, NULL,
				MKDEV(32, device_index), NULL, device_name);
	gi->device_index = device_index;
	if (IS_ERR(gi->android_device))
		return PTR_ERR(gi->android_device);

	dev_set_drvdata(gi->android_device, gi);

	attrs = android_usb_attributes;
	while ((attr = *attrs++)) {
		int err;

		err = device_create_file(gi->android_device, attr);
		if (err) {
			device_destroy(gi->android_device->class,
				       gi->android_device->devt);
			return err;
		}
	}
...
static void android_device_destroy(struct gadget_info *gi)
{
	...
	attrs = android_usb_attributes;
	while ((attr = *attrs++))
		device_remove_file(gi->android_device, attr);
	device_destroy(gi->android_device->class, gi->android_device->devt);
	clear_device_index(gi->device_index);
}
#else
