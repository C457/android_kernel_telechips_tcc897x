...
{
   struct snd_uac2_chip *uac2 = container_of(data, struct snd_uac2_chip, work);
...
static struct usb_interface_assoc_descriptor iad_desc = {
	...
	//.bFirstInterface = 0, // 2018.01.12 [STMS_137991] no sound when connecting iOS 11 iPhone
	.bInterfaceCount = 3,
...
afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
{
	...
	iad_desc.bFirstInterface = ret; // 2018.01.12 [STMS_137991] no sound when connecting iOS 11 iPhone
	agdev->ac_intf = ret;
	agdev->ac_alt = 0;
	...
	prm->rbuf = kzalloc(/*prm->max_psize*/PRM_MAX_SIZE * USB_XFERS, GFP_KERNEL);
	if (!prm->rbuf) {
		prm->max_psize = 0;
		goto err_free_descs;
	}
...
afunc_set_alt(struct usb_function *fn, unsigned intf, unsigned alt)
{
	...
	for (i = 0; i < USB_XFERS; i++) {
		if (!prm->ureq[i].req) {
			...
			req->buf = prm->rbuf + i * PRM_MAX_SIZE/*prm->max_psize*/;
		}
