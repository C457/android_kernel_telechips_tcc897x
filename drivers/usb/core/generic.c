...
int usb_choose_configuration(struct usb_device *udev)
{
	...
	if(udev->descriptor.idVendor == 0x5ac){
		...
		if ((fHasAudioClass == 1) || (fHasHidClass == 1))
		{
			//skip set configuration step for time-saving carplay connection.
			printk("[%s]AUDIO/HID support\n", __func__);
			return -1;
		}
		else
		{
			printk("[%s]MASS STORAGE support\n", __func__);
		}
	}
#endif
