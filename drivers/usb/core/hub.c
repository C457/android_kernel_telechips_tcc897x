...
static ssize_t force_hub_port_reset_store(struct device *_dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct usb_device *udev = to_usb_device(_dev);
...
static ssize_t usbstatus_show(struct device *_dev,
		struct device_attribute *attr,
		char *buf)
{
	u32 nReg = 0x00;
	...
	nReg = readl((void*)tcc_p2v(0x71000420));
	printk(KERN_DEBUG "0x71000420 (0x%x) \r\n",nReg);
	if (nReg & 0x01)
		sprintf(buf, "%d\n",1);

	nReg = readl((void*)tcc_p2v(0x71000430));
	printk(KERN_DEBUG "0x71000430 (0x%x) \r\n",nReg);
	if (nReg & 0x01)
		sprintf(buf, "%d\n",1);
...
static DEVICE_ATTR(force_hub_reset, 0664, NULL, force_hub_port_reset_store);
static DEVICE_ATTR(usbstatus, 0220, usbstatus_show, NULL);

/* Group all the USB bus attributes */
static struct attribute *force_hub_func_attrs[] = {
	&dev_attr_force_hub_reset.attr,
	&dev_attr_usbstatus.attr,
	NULL,
};

static struct attribute_group force_hub_func_attr_group = {
	.name = NULL,	/* we want them in the same directory */
	.attrs = force_hub_func_attrs,
};
...
#ifdef CONFIG_DAUDIO
static unsigned sysfs_force_hub_reset;
#endif /* CONFIG_DAUDIO */
...
#ifdef CONFIG_DAUDIO
	if (sysfs_force_hub_reset == 1)
		sysfs_remove_group(&hdev->dev.kobj, &force_hub_func_attr_group);
#endif /* CONFIG_DAUDIO */
...
#ifdef CONFIG_DAUDIO
	sysfs_force_hub_reset = 0;
	err = sysfs_create_group(&hdev->dev.kobj, &force_hub_func_attr_group);
	if (err < 0)
		pr_warn("cannot create force hub reset group %d\n", err);
	else
		sysfs_force_hub_reset = 1;
#endif /* CONFIG_DAUDIO */
...
#if 0 // in order to prevent autosuspend
	/*
...
/* TCC OTG PET Test */
	...
	dev_emerg(&udev->dev, "USB disconnect(%s), device number %d\n", udev->product,
			udev->devnum);
#else
	dev_info(&udev->dev, "USB disconnect(%s), device number %d\n", udev->product,
			udev->devnum);
			...
			if (desc->bmAttributes & USB_OTG_HNP) {
				...
				err = usb_control_msg(udev,
					...
					bus->b_hnp_enable
						...
						: USB_DEVICE_A_HNP_SUPPORT,//USB_DEVICE_A_ALT_HNP_SUPPORT,
...
/* TCC OTG PET Test */
	...
	else if (udev->portnum == udev->bus->otg_port)
	{
		...
		if(udev->bus->b_hnp_enable)
			timeout = THOST_REQ_POLL;
		else if(udev->bus->is_b_host)
			timeout = OTG_TTST_SUSP;
		else
			return 0;

		udev->bus->hnp_work->udev = udev;
		ret = queue_delayed_work(udev->bus->hnp_wq ,
			(struct delayed_work *) udev->bus->hnp_work, msecs_to_jiffies(timeout));
		//printk("\x1b[1;31m[%s:%d] Schedule Suspend for HNP (%d msec) (ret = %d)\x1b[0m\n",__func__, __LINE__,timeout,ret);
	}
#endif /* CONFIG_TCC_DWC_HS_ELECT_TST */
		...
		__must_hold(&port_dev->status_lock)
{
	...
	dev_err(&port_dev->dev, "status %04x, change %04x, %s\n", portstatus,
