...
#ifdef CONFIG_USB_SERIAL_LGIT_9x28_MODEM
	if ( dev->descriptor.idVendor == 0x1d74 &&
	    (dev->descriptor.idProduct == 0x3100 || dev->descriptor.idProduct == 0x3102) )
	{
		iface_desc = interface->cur_altsetting;
/*
		if (iface_desc->desc.bInterfaceNumber == 1)
		{
			dev_info(ddev, "Not bind LG Innotek ADB interface to USB serial\n");
			usb_serial_put(serial);
			module_put(type->driver.owner);
			return -EIO;
		}
*/
		if (iface_desc->desc.bInterfaceNumber == 7)
		{
			dev_info(ddev, "Not bind LG Innotek Rmnet interface to USB serial\n");
			usb_serial_put(serial);
			module_put(type->driver.owner);
			return -EIO;
		}
