...
#if defined(CONFIG_ARCH_TCC897X) && !defined(CONFIG_TCC_ADC_NO_TOUCHSCREEN)
...
#if defined(CONFIG_TCC_ADC_NO_TOUCHSCREEN)
		/* adctsc control */
...
unsigned long tcc_adc_getdata(struct tcc_adc_client *client)
{
	...
	//printk(KERN_ERR "%s: %u (0x%08X)\n", __func__, client->data, client->data);
...
int tcc_adc_is_12bit_res(void)
{
	struct adc_device *adc = adc_dev;
	unsigned reg_values = adc_readl(adc->regs+ADCCON_REG);

	return reg_values & ADCCON_12BIT_RES;
}
EXPORT_SYMBOL_GPL(tcc_adc_is_12bit_res);
...
{
	printk("\x1b[1;33m[%s:%d]\x1b[0m\n", __func__, __LINE__);
	return platform_driver_register(&tcc_adc_driver);
}
