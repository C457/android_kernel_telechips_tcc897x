...
inline static int tcc898x_set_eint(void __iomem *base, unsigned bit, int extint)
{
	...
	extintr_match[extint].port_base = base;
...
{
	void __iomem *reg = base + GPIO_INPUT_BUFFER_ENABLE;
	unsigned int data;
...
static void tcc898x_gpio_pull_enable(void __iomem *base, unsigned offset,
				     int enable)
{
	void __iomem *reg = base + GPIO_PULL_ENABLE;
	int data;
...
static void tcc898x_gpio_pull_select(void __iomem *base, unsigned offset,
				     int up)
{
	void __iomem *reg = base + GPIO_PULL_SELECT;
	int data;
...
static int tcc898x_gpio_to_irq(void __iomem *base, unsigned offset)
{
	...
	for (i=0; i<EINT_MAX_SIZE ; i++) {
		if (extintr_match[i].used && (extintr_match[i].port_base == base)
			...
			int config)
{
	...
	case TCC_PINCONF_PARAM_INPUT_BUFFER_ENABLE:
		tcc898x_gpio_input_buffer_set(base, offset, 1);
		break;

	case TCC_PINCONF_PARAM_INPUT_BUFFER_DISABLE:
		tcc898x_gpio_input_buffer_set(base, offset, 0);
		break;
	}
	return 0;
}
...
static struct tcc_pinctrl_soc_data tcc898x_pinctrl_soc_data = {
	.pin_configs = tcc898x_pin_configs,
	.nconfigs = ARRAY_SIZE(tcc898x_pin_configs),
	.ops = &tcc898x_ops,
};
...
static int tcc898x_pinctrl_probe(struct platform_device *pdev)
{
	...
	return tcc_pinctrl_probe(pdev, &tcc898x_pinctrl_soc_data);
}
