...
int qmi_ctl_send_and_forget(struct rmnet_context *context, void *msg, int msglen)
{
	...
	if(context == NULL){
		printk(KERN_ERR "rmnet qmi_ctl_send_and_forget - device context is null. end\n");
		return -EINVAL;
	}
	...
	if(g_device_contexts[context->dev_instance] == NULL){
		printk(KERN_ERR "rmnet qmi_ctl_send_and_forget - device instance is null. end\n");
		return -EINVAL;
	}
...
int qmi_ctl_send_and_wait(struct rmnet_context *context, void *msg, int msglen, u8 txid, void *buf, int buflen)
{
	...
	if(context == NULL){
		printk(KERN_ERR "rmnet qmi_ctl_send_and_wait - device context is null. end\n");
		return -EINVAL;
	}


	if(g_device_contexts[context->dev_instance] == NULL){
		printk(KERN_ERR "rmnet qmi_ctl_send_and_wait - device instance is null. end\n");
		return -EINVAL;
	}
	...
	if (tx_item == NULL) {
		printk(KERN_ERR "%s : control txitem memory alloc error\n", NET_DEVICE_NAME);
		status = -ENOMEM;
		goto exit;
	}
...
int qmi_ctl_get_version_info(struct rmnet_context *context)
{
	...
	// send failure
	if (status < 0) {
		printk(KERN_ERR "%s : qmi get version error\n", NET_DEVICE_NAME);
		goto exit;
	} else if (status == 0) {
		printk(KERN_ERR "%s : qmi get version - no response\n", NET_DEVICE_NAME);
		goto exit;
	}
	...
	if (response->qmi_result != 0) {
		printk(KERN_ERR "%s : qmi get version result error\n", NET_DEVICE_NAME);
		status = -1;
		goto exit;
	}
...
int qmi_ctl_get_client_id(struct rmnet_context *context, u8 service_type, u8 *client_id)
{
	...
	// send failure
	if (status < 0) {
		printk(KERN_ERR "%s : qmi get client id error\n", NET_DEVICE_NAME);
		goto exit;
	} else if (status == 0) {
		printk(KERN_ERR "%s : qmi get client id - no response\n", NET_DEVICE_NAME);
		goto exit;
	}
	...
	// qmi result error
	if (response->qmi_result != 0) {
		printk(KERN_ERR "%s : qmi get client id result error\n", NET_DEVICE_NAME);
		status = -1;
		goto exit;
	}
	...
	if (assigned_cid->qmi_svc_type != service_type) {
		printk(KERN_ERR "%s : get client id error - service type mismatched\n", NET_DEVICE_NAME);
		status = -1;
		goto exit;
	}
...
int qmi_ctl_release_client_id(struct rmnet_context *context, u8 service_type, u8 client_id)
{
	...
	// send failure
	if (status < 0) {
		printk(KERN_ERR "%s : qmi release client id error\n", NET_DEVICE_NAME);
	}
...
int qmi_ctl_sync(struct rmnet_context *context)
{
	...
	// send failure
	if (status < 0) {
		printk(KERN_ERR "%s : qmi sync error\n", NET_DEVICE_NAME);
		goto exit;
	} else if (status == 0) {
		printk(KERN_ERR "%s : qmi sync - no response\n", NET_DEVICE_NAME);
		goto exit;
	}
	...
	// qmi result error
	if (response->qmi_result != 0) {
		printk(KERN_ERR "%s : qmi sync result error\n", NET_DEVICE_NAME);
		status = -1;
		goto exit;
	}
...
int qmi_ctl_set_instance_id(struct rmnet_context *context)
{
	...
	// send failure
	if (status < 0) {
		printk(KERN_ERR "%s : qmi set instance id error\n", NET_DEVICE_NAME);
		goto exit;
	} else if (status == 0) {
		printk(KERN_ERR "%s : qmi set instance id - no response\n", NET_DEVICE_NAME);
		goto exit;
	}
	...
	// qmi result error
	if (response->qmi_result != 0) {
		printk(KERN_ERR "%s : qmi set instance id result error\n", NET_DEVICE_NAME);
		status = -1;
		goto exit;
	}
