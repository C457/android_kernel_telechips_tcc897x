...
int qmi_svc_send_and_forget(struct rmnet_context *context, void *msg, int msglen)
{
	...
	if(context == NULL){
		printk(KERN_ERR "rmnet qmi_svc_send_and_forget - device context is null. end\n");
		return -EINVAL;
	}


	if(g_device_contexts[context->dev_instance] == NULL){
		printk(KERN_ERR "rmnet qmi_svc_send_and_forget - device instance is null. end\n");
		return -EINVAL;
	}
	...
	if (status < 0) {
		printk(KERN_ERR "%s : usb service message send error\n", NET_DEVICE_NAME);
	}
...
int qmi_svc_send_and_wait(struct rmnet_context *context, u8 service_type, u8 client_id, void *msg, int msglen, u8 txid, void *buf, int buflen)
{
	...
	if (tx_item == NULL) {
		printk(KERN_ERR "%s : service txitem memory alloc error\n", NET_DEVICE_NAME);
		status = -ENOMEM;
		goto exit;
	}
...
int qmi_wda_set_data_format(struct rmnet_context *context)
{
	...
	if (request_size == sizeof(struct qmi_wda_set_data_format_req)) {
		printk(KERN_ERR "%s : there is no data format data\n", NET_DEVICE_NAME);
		return 0;
	}
	...
	// send failure
	if (status <= 0) {
		printk(KERN_ERR "%s : qmi wda set data format error\n", NET_DEVICE_NAME);
		goto exit;
	}
	...
	if (response->qmi_result != 0) {
		printk(KERN_ERR "%s : qmi wda set data format result error\n", NET_DEVICE_NAME);
		status = -1;
		goto exit;
	}
...
int qmi_wda_set_qmap_settings(struct rmnet_context *context)
{
	...
	if (request_size == sizeof(struct qmi_wda_set_qmap_settings_req)) {
		printk(KERN_ERR "%s : there is no qmap setting data\n", NET_DEVICE_NAME);
		return 0;
	}
	...
	// send failure
	if (status <= 0) {
		printk(KERN_ERR "%s : qmi wda set qmap settings error\n", NET_DEVICE_NAME);
		goto exit;
	}
	...
	// qmi result error
	if (response->qmi_result != 0) {
		printk(KERN_ERR "%s : qmi wda set qmap settings result error\n", NET_DEVICE_NAME);
		status = -1;
		goto exit;
	}
 ...
 *   > 0 : success
 */
int qmi_wds_bind_mux_data_port(struct rmnet_context *context, u16 transaction_id, u8 client_id)
{
	int status = 0, request_size = 0;
	char *buffer = NULL, *req_offset = NULL, *read_offset = NULL;
	...
	if (request_size == sizeof(struct qmi_wds_mux_data_port_req)) {
		printk(KERN_ERR "%s : there is no bind mux port data\n", NET_DEVICE_NAME);
		return 0;
	}
	...
	// send failure
	if (status <= 0) {
		printk(KERN_ERR "%s : qmi wds bind mux data port error\n", NET_DEVICE_NAME);
		goto exit;
	}
	...
	// qmi result error
	if (response->qmi_result != 0) {
		printk(KERN_ERR "%s : qmi wds bind mux data port result error\n", NET_DEVICE_NAME);
		status = -1;
		goto exit;
	}
...
int qmi_wds_set_client_ip_family(struct rmnet_context *context, u16 transaction_id, u8 client_id, u8 ip_pref)
{
	...
	// send failure
	if (status <= 0) {
		printk(KERN_ERR "%s : qmi wds set client ip family pref error\n", NET_DEVICE_NAME);
		goto exit;
	}
	...
	// qmi result error
	if (response->qmi_result != 0) {
		printk(KERN_ERR "%s : qmi wds set client ip family pref result  error\n", NET_DEVICE_NAME);
		status = -1;
		goto exit;
	}
...
int qmi_wds_indication_register(struct rmnet_context *context, u16 transaction_id, u8 client_id)
{
	...
	// send failure
	if (status <= 0) {
		printk(KERN_ERR "%s : qmi wds indication register error\n", NET_DEVICE_NAME);
		goto exit;
	}
	...
	// qmi result error
	if (response->qmi_result != 0) {
		printk(KERN_ERR "%s : qmi wds indication register result  error\n", NET_DEVICE_NAME);
		status = -1;
		goto exit;
	}
...
int qmi_wds_set_event_report(struct rmnet_context *context, u16 transaction_id, u8 client_id)
{
	...
	// send failure
	if (status < 0) {
		printk(KERN_ERR "%s : qmi wds set event report error\n", NET_DEVICE_NAME);
	}
...
int qmi_wds_get_runtime_settings(struct rmnet_context *context, u16 transaction_id, u8 client_id)
{
	...
	// send failure
	if (status <= 0) {
		printk(KERN_ERR "%s : qmi wds get runtime settings error\n", NET_DEVICE_NAME);
		goto exit;
	}
	...
	// qmi result error
	if (response->qmi_result != 0) {
		printk(KERN_ERR "%s : qmi wds get runtime settings result error\n", NET_DEVICE_NAME);
		status = -1;
		goto exit;
	}
...
int qmi_wds_get_runtime_settings_async(struct rmnet_context *context, u16 transaction_id, u8 client_id, u32 req_settings)
{
	...
	// send failure
	if (status < 0) {
		printk(KERN_ERR "%s : qmi wds get current channel rate error\n", NET_DEVICE_NAME);
	}
...
int qmi_wds_get_pkt_srvc_status(struct rmnet_context *context, u8 ip_family)
{
	...
	// send failure
	if (status < 0) {
		printk(KERN_ERR "%s : qmi wds get pkt srvc status error\n", NET_DEVICE_NAME);
	}
...
ssize_t qmi_svc_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	pr_debug(NET_DEVICE_NAME " QMI service read (filp=0x%x context=0x%x length=%d)\n", (unsigned int)filp, (unsigned int)svc_context, count);

	if (context->rmnet_status & RMNET_STATUS_REMOVED) {
		printk(KERN_ERR "%s : read error : device already removed \n", NET_DEVICE_NAME);
		return -ENODEV;
	}
	mutex_lock(&context->cxt_lock);
	...
	if (node != &svc_context->data_queue) {
		...
		printk(KERN_ERR "%s : there is no received message\n", NET_DEVICE_NAME);
...
ssize_t qmi_svc_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	...
	pr_debug(NET_DEVICE_NAME " QMI service write (filp=0x%x context=0x%x length=%d)\n", (unsigned int)filp, (unsigned int)svc_context, count);

	if (RMNET_STATUS_REMOVED & context->rmnet_status) {
		printk(KERN_ERR "%s : write error : device already removed\n", NET_DEVICE_NAME);
		return -ENODEV;
	}
	...
	if (msg == NULL) {
		pr_err(NET_DEVICE_NAME " message buffer allocation failed\n");
		return -1;
	}
...
long qmi_svc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (RMNET_STATUS_REMOVED & context->rmnet_status) {
		printk(KERN_ERR "%s : ioctl error : device already removed\n", NET_DEVICE_NAME);
		return -ENODEV;
	}
	...
	case QMI_SVC_IOCTL_SET_SERVICE :
		if (arg == 0) {
			printk(KERN_ERR "%s : set service error : no service type\n", NET_DEVICE_NAME);
			result = -EINVAL;
		}
		...
		if (svc_context->service_type != 0) {
			printk(KERN_ERR "%s : set service error : not support service type\n", NET_DEVICE_NAME);
			result = -EBADR;
		}
		...
		if (result > 0) {
			...
			if (service_type == RMNET_QMI_VERSION_WDS) {
				if (qmi_wds_bind_mux_data_port(context, 0xF0, svc_context->client_id) < 0) {
					printk(KERN_ERR "%s : client bind mux data port error\n", NET_DEVICE_NAME);
				}
			}
		}
...
int qmi_svc_open(struct inode *inode, struct file *filp)
{
	...
	pr_debug(NET_DEVICE_NAME " QMI service open (inode=0x%x filp=0x%x)\n", (unsigned int)inode, (unsigned int)filp);
	if (RMNET_STATUS_REMOVED & context->rmnet_status) {
		printk(KERN_ERR "%s : open error : device already removed\n", NET_DEVICE_NAME);
		result = -ENODEV;
		goto exit;
	}
	...
	if ((context->rmnet_status & RMNET_STATUS_VERSION) == 0 ||
		...
		printk(KERN_ERR "%s : open error : not yet initialized\n", NET_DEVICE_NAME);
		result = -ENODEV;
		goto exit;
	}
	...
	if (filp->private_data == NULL) {
		printk(KERN_ERR "%s : open error : can't alloc memory\n", NET_DEVICE_NAME);
		result = -ENOMEM;
		goto exit;
	}
	...
	svc_context->dev_instance = context->dev_instance;
	atomic_set(&svc_context->ref_cnt, 1);

	pr_debug(NET_DEVICE_NAME " add service context 0x%08x(file=0x%x)\n", (unsigned int)svc_context, (unsigned int)filp);
...
int qmi_svc_flush(struct file *filp, fl_owner_t id)
{
	...
	printk(KERN_NOTICE "rmnet qmi_svc_flush start\n");


	if(filp == NULL){
		printk(KERN_ERR "rmnet qmi_svc_flush - filp is null. end\n");
		return 0;
	}
	...
	if(svc_context == NULL){
		printk(KERN_ERR "rmnet qmi_svc_flush - service context is null. end\n");
		return 0;
	}

	if(g_device_contexts[svc_context->dev_instance] == NULL){
		printk(KERN_ERR "rmnet qmi_svc_flush - device instance is null. end\n");
		return 0;
	}
	...
	if(context == NULL){
		printk(KERN_ERR "rmnet qmi_svc_flush - device context is null. end\n");
		return 0;
	}

	pr_debug(NET_DEVICE_NAME " QMI service flush (filp=0x%x context=0x%x)\n", (unsigned int)filp, (unsigned int)svc_context);
	...
	if (svc_context->read_wait_active > 0 &&
		...
		pr_debug(NET_DEVICE_NAME " wake up device(context:0x%x) for read waiting by flush\n", (unsigned int)svc_context);
		svc_context->read_wait_active = 0;
	...
	while (next_node != &svc_context->data_queue) {
		...
		remove_list(node);
		data_list = (struct qmi_svc_data_list *)container_of(node, struct qmi_svc_data_list, list);
		pr_debug(NET_DEVICE_NAME " remove remain data list 0x%08x\n", (unsigned int)data_list);
		kfree(data_list);
	}
	...
	pr_debug(NET_DEVICE_NAME " remove service context 0x%08x\n", (unsigned int)svc_context);
...
int qmi_svc_release(struct inode *inode, struct file *filp)
{
	...
	if(svc_context == NULL){
		pr_warn(NET_DEVICE_NAME " QMI service release service context is null - return\n");
		return 0;
	}

	pr_debug(NET_DEVICE_NAME " QMI service release (filp=0x%x context=0x%x)\n", (unsigned int)filp, (unsigned int)svc_context);
	...
	while (node->next != &context->svc_list_head) {
		...
		if (header->msg_header.service_type == svc_context->service_type) {
			...
			if (header->msg_header.client_id == svc_context->client_id || header->msg_header.client_id == 0xFF) {
				...
				pr_debug(NET_DEVICE_NAME " response data 0x%08x append to 0x%08x\n", (unsigned int)data_list, (unsigned int)svc_context);
