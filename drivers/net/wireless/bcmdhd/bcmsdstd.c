 ...
 *
 * $Id: bcmsdstd.c 692354 2018-07-09 11:34:16Z $
 */
...
sdstd_client_init(sdioh_info_t *sd)
{
	...
	/* Setting drive-strength for 89359 SDIO chips to use type-A based on DVT results */
	status = sdstd_card_regread(sd, SDIO_FUNC_1, CHIP_CONTROL_BASE, 4, &chipid);
	if (BCME_OK != status) {
		sd_err(("%s: Accessing Chip ID failed - status: %d\n", __FUNCTION__, status));
		return BCME_SDIO_ERROR;
	}
	if (BCM4349_CHIP(chipid & CHIP_CONTROL_CHIPID_MASK)) {
		dhd_sdiod_uhsi_ds_override[0] = 'C';
	}
...
sdstd_3_clk_tuning(sdioh_info_t *sd, uint32 sd3ClkMode)
{
	...
	do {
		...
		if (brr_count == CLKTUNING_MAX_BRR_RETRIES) {
			if (brr_timedout_count++ > MAX_TUNING_ITERS) {
				sd_err(("%s: TUNINGFAILED: BRR response timedout! exit tuning\n",
					__FUNCTION__));
				val1 = sdstd_rreg16(sd, SD3_HostCntrl2);
				val1 = SFIELD(val1, HOSTCtrl2_EXEC_TUNING, 0);
				sdstd_wreg16(sd, SD3_HostCntrl2, val1);
				...
				/* If there is a BRR timedout then retry tuning
				 * for next clock phase
				 */
				sd_err(("%s: BRR response timedout! retry tuning for %d\n",
					__FUNCTION__, brr_timedout_count));
				continue;
			}
		}
	...
	if (brr_timedout_count) {
		sd_err(("Phase passed info: 0x%x\n", (val2>>8)& 0x3F));
		sd_err(("Phase selected post tune: 0x%x\n", phase_info_local));
	} else {
	...
	switch (sd->sd_dma_mode) {
		case DMA_MODE_ADMA2:
			...
			while (adma2_dscr_table->len_attr & ADMA2_ATTRIBUTE_VALID) {
				...
				snprintf(flags_str, sizeof(flags_str), "%s%s%s%s",
		...
		case DMA_MODE_ADMA1:
			...
			for (i = 0; adma1_dscr_table->phys_addr_attr & ADMA2_ATTRIBUTE_VALID; i++) {
				...
				snprintf(flags_str, sizeof(flags_str) - 1, "%s%s%s%s",
					((flags & ADMA2_ATTRIBUTE_ACT_LINK) ==
					ADMA2_ATTRIBUTE_ACT_LINK) ? "LINK " :
					((flags & ADMA2_ATTRIBUTE_ACT_LINK) ==
					ADMA2_ATTRIBUTE_ACT_TRAN) ? "TRAN " :
