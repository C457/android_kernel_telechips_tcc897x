...
//FEATURE_MOBIS_WIFI_BROADCOM
//ADDED_CODE_FOR_MOBIS_DEVICE(+)
#if 0 //jaehyungkim@mobis.co.kr 2016.11.15 temp
...
//=============================================================
	...
	if (err < 0)
		DHD_ERROR(("%s: regulator enable/disable failed , error val = %d", __FUNCTION__,err));
	err = 0;
//=============================================================
#else
	struct wifi_platform_data *plat_data;
...
//FEATURE_MOBIS_WIFI_BROADCOM
//ADDED_CODE_FOR_MOBIS_DEVICE(+)
#if 1 //jaehyungkim@mobis.co.kr 2016.11.15 temp
//ADDED_CODE_FOR_MOBIS_DEVICE(-)
	if (on && !err)
		is_power_on = TRUE;
	else
...
int wifi_platform_bus_enumerate(wifi_adapter_info_t *adapter, bool device_present)
{
	...
	if (plat_data->set_carddetect) {
	    ...
	    DHD_ERROR(("%s plat_data->set_carddetect %d\n", __FUNCTION__, err));
	}
	return err;
}
//FEATURE_MOBIS_WIFI_BROADCOM
...
#endif    
	of_match_device(wifi_device_dt_match, &pdev->dev);
	wifi_reg_on_port = of_get_named_gpio(pdev->dev.of_node, "wlreg_on-gpio", 0);
	if (wifi_reg_on_port < 0) {
        DHD_ERROR(("%s gpio information is incorrect\n", __FUNCTION__));
        return -1;
	}    
	...
	for (i = 0; i < dhd_wifi_platdata->num_adapters; i++) {
		...
		do {
			...
			if (err) {
				...
				wifi_platform_set_power(adapter, FALSE, WIFI_TURNOFF_DELAY);
				DHD_ERROR(("%s wifi_platform_set_power fail(%d)\n",__FUNCTION__, err));
				continue;
			} else {
				wifi_platform_bus_enumerate(adapter, TRUE);
				DHD_ERROR(("%s wifi_platform_bus_enumerate\n",__FUNCTION__));
				err = 0;
			}

			if (down_timeout(&dhd_chipup_sem, msecs_to_jiffies(POWERUP_WAIT_MS)) == 0) {
				DHD_ERROR(("%s down_timeout wait dhd_chipup_sem\n",__FUNCTION__));
...
fail:
	/* power down all adapters */
	DHD_ERROR(("%s: power down all adapters \n", __FUNCTION__));
	for (i = 0; i < dhd_wifi_platdata->num_adapters; i++) {
		adapter = &dhd_wifi_platdata->adapters[i];
		wifi_platform_set_power(adapter, FALSE, WIFI_TURNOFF_DELAY);
		wifi_platform_bus_enumerate(adapter, FALSE);
