...
static void init_rx_dma_desc_rings(struct net_device *dev, unsigned int ch)
{
	...
	for (i = 0; i < rxsize; i++) {
		...
		if (unlikely(buff2_needed))
			...
			DBG(probe, INFO, "[%p]\t[%p]\t[%x]\n", priv->rx_dma_ch[ch].rx_skbuff[i],
				priv->rx_dma_ch[ch].rx_skbuff[i]->data, priv->rx_dma_ch[ch].rx_skbuff_dma[i]);
	}
...
static void tcc_gmac_dma_operation_mode(struct tcc_gmac_priv *priv, unsigned int ch)
{
	if ((priv->dev->mtu <= ETH_DATA_LEN) && (tx_coe)) {
		priv->hw->dma[ch]->dma_mode((volatile void*)priv->dev->base_addr, SF_DMA_MODE, SF_DMA_MODE);
		tc = SF_DMA_MODE;
		priv->tx_coe = HW_CSUM;
	} else {
		/* Checksum computation is performed in software. */
		priv->hw->dma[ch]->dma_mode((volatile void*)priv->dev->base_addr, tc, SF_DMA_MODE);
...
static inline void tcc_gmac_enable_rx(volatile void *ioaddr)
{
...
static inline void tcc_gmac_enable_tx(volatile void *ioaddr)
{
	u32 value = readl(ioaddr + MAC_CTRL_REG);
	value |= MAC_ENABLE_TX;
...
static inline void tcc_gmac_disable_rx(volatile void *ioaddr)
{
	u32 value = readl(ioaddr + MAC_CTRL_REG);
	value &= ~MAC_RNABLE_RX;
	writel(value, ioaddr + MAC_CTRL_REG);
...
static inline void tcc_gmac_disable_tx(volatile void *ioaddr)
{
	u32 value = readl(ioaddr + MAC_CTRL_REG);
	value &= ~MAC_ENABLE_TX;
	writel(value, ioaddr + MAC_CTRL_REG);
...
{
	priv->hw->dma[ch]->enable_dma_irq((volatile void*)priv->dev->base_addr);
}

static inline void tcc_gmac_disable_irq(struct tcc_gmac_priv *priv, unsigned int ch)
{
	priv->hw->dma[ch]->disable_dma_irq((volatile void*)priv->dev->base_addr);
}
...
//	struct rx_dma_ch_t *rx_dma = &priv->rx_dma_ch[ch];
    ...
    volatile void *ioaddr = (volatile void*)priv->dev->base_addr;
	struct netdev_queue *txq = netdev_get_tx_queue(priv->dev, ch);
	//unsigned long flags;
...
static void tcc_gmac_tx_err(struct tcc_gmac_priv *priv)
{
	...
	for (i=0; i<NUMS_OF_DMA_CH; i++) {
		priv->hw->dma[i]->stop_tx((volatile void*)priv->dev->base_addr);
		...
		priv->hw->dma[i]->start_tx((volatile void*)priv->dev->base_addr);
	}
...
static int tcc_gmac_stop(struct net_device *dev)
{
	...
	for (i=0; i < NUMS_OF_DMA_CH; i++) {
		priv->hw->dma[i]->stop_tx((volatile void*)dev->base_addr);
		priv->hw->dma[i]->stop_rx((volatile void*)dev->base_addr);
	}
	...
	tcc_gmac_disable_tx((volatile void*)dev->base_addr);
	tcc_gmac_disable_rx((volatile void*)dev->base_addr);
...
//	spin_lock_irqsave(&priv->lock, flags);
	...
	priv->hw->dma[ch]->enable_dma_transmission((volatile void*)dev->base_addr);
	//spin_unlock_irqrestore(&priv->lock, flags);
	return NETDEV_TX_OK;
}
...
static int ioctl_hwtstamp(struct net_device *dev, struct ifreq *rq)
{
	...
	if (priv->hw->ptp->snapshot_mode((volatile void*)dev->base_addr, config.rx_filter) < 0)
		return -EINVAL;
...
static int tcc_gmac_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (tcc_power_state == TCC_PM_STATE_SLEEP) {
		...
		/* Stop TX/RX DMA */
		priv->hw->dma_ch0->stop_tx((volatile void*)dev->base_addr);
		priv->hw->dma_ch0->stop_rx((volatile void*)dev->base_addr);
		/* Clear the Rx/Tx descriptors */
		...
		if (device_may_wakeup(&(pdev->dev))) {
			...
			if (priv->wolenabled == PMT_SUPPORTED)
				priv->hw->mac->pmt((volatile void*)dev->base_addr,
						priv->wolopts);
		} else {
			tcc_gmac_disable_rx((volatile void*)dev->base_addr);
		}
...
static int tcc_gmac_resume(struct platform_device *pdev)
{
	...
	if (device_may_wakeup(&(pdev->dev)))
		if (priv->wolenabled == PMT_SUPPORTED)
			priv->hw->mac->pmt((volatile void*)dev->base_addr, 0);
...
static int tcc_gmac_probe(struct platform_device *pdev)
{
	...
	unsigned char default_mac_addr[ETH_ALEN]={0x00,0x12,0x34,0x56,0x78,0x00};
	int ret = 0;
	...
	priv->hw = tcc_gmac_setup((volatile void*)dev->base_addr, tca_gmac_get_hsio_clk(&priv->dt_info));
	if (!priv->hw) {
		return -ENOMEM;
	}
