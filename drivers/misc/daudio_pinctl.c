...
static D_AUDIO_GPIO_LIST gpio_list[] =
{
    ...
    {ENABLE_TPEG_SPI_INT,       TCC_GPC(21),    "ENABLE_TPEG_SPI_INT"},//M136_SATURN_SPI_INT 
...
static int daudio_pinctl_fill_parameter(struct st_ext_int_info *int_info)
{
	...
	switch (int_info->extint) {
		case INT_EINT0:  break;
		case INT_EINT1:  break;
		case INT_EINT2:  break;
		case INT_EINT3:  break;
		case INT_EINT4:  break;
		case INT_EINT5:  break;
		case INT_EINT6:  break;
		case INT_EINT7:  break;
		case INT_EINT8:  break;
...
static void daudio_pinctl_manage_interrupt(int gpio, int value)
{
	...
	switch(cmd)
	{
		...
		case ENABLE_CMMB_SPI_UPDATE:
		{
			...
			break;
#endif
#if 0  //Add feature DAB
		case ENABLE_DAB_SPI_INT:
		{
			if(value)
				ret = daudio_pinctl_enable_interrupt(TCC_GPE(7), INT_EINT11);
			else
				ret = daudio_pinctl_disable_interrupt(TCC_GPE(7), INT_EINT11);
		}
			break;
#endif
		case ENABLE_DAB_SPI_INT:
