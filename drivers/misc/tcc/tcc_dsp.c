		...
		case IOCTL_TCC_CONTROL_SET_PARAM:
			{
				...
				ret = tcc_control_set_param(&dsp->am3d, &param);
		...
		case IOCTL_TCC_CONTROL_GET_PARAM:
			{
				...
				ret = tcc_control_get_param(&dsp->am3d, &param);
		...
		case IOCTL_TCC_AM3D_SET_PARAM:
			{
				...
				ret = tcc_am3d_set_param(&dsp->am3d, &param);
		...
		case IOCTL_TCC_AM3D_GET_PARAM:
			{
				...
				ret = tcc_am3d_get_param(&dsp->am3d, &param);
		...
		case IOCTL_TCC_AM3D_SET_PARAM_TABLE:
			{
				...
				ret = tcc_am3d_set_param_tbl(&dsp->am3d, &param_tbl);
		...
		case IOCTL_TCC_AM3D_GET_PARAM_TABLE:
			{
				...
				ret = tcc_am3d_get_param_tbl(&dsp->am3d, &param_tbl);

				if (copy_to_user((void*)arg, &param_tbl, sizeof(struct tcc_am3d_param_tbl_t)))
					break;
			}
			break;
		case IOCTL_TCC_AM3D_CREATE_AND_ADD_FILTER:
			{
				struct tcc_am3d_eq_filter_t *eq_filter;

				eq_filter = (struct tcc_am3d_eq_filter_t*)kmalloc(sizeof(struct tcc_am3d_eq_filter_t), GFP_KERNEL);
				if (copy_from_user(eq_filter, (void*)arg, sizeof(struct tcc_am3d_eq_filter_t)))
					break;

				ret = tcc_am3d_create_and_add_filter(&dsp->am3d, eq_filter);

				kfree(eq_filter);
			}
			break;
		case IOCTL_TCC_AM3D_SELECT_FILTER_BY_INDEX:
			{
				struct tcc_am3d_select_filter_by_index_t select;

				if (copy_from_user(&select, (void*)arg, sizeof(struct tcc_am3d_select_filter_by_index_t)))
					break;

				ret = tcc_am3d_select_filter_by_index(&dsp->am3d, &select);

				if (copy_to_user((void*)arg, &select, sizeof(struct tcc_am3d_select_filter_by_index_t)))
					break;
			}
			break;
		default:
...
static int tcc_dsp_probe(struct platform_device *pdev)
{
	...
	tcc_am3d_init(&dsp->am3d, &pdev->dev);
...
#endif
            tc_dsp_setCurrAudioBuffer(I2S0params, ipc_data->param[1],1);
            {
                void tcc_dma_done_handler(int playback);
                tcc_dma_done_handler(1);
            }
        }
    }
    ...
    if(ipc_data->type == INFO_TYPE){
        if(ipc_data->param[0] == ALSA_IN_BUF_PARAM){
            ...
            printk("id:0x%08x, type:0x%08x,ALSA_IN_BUF_PARAM = 0x%08x,CurrAddr 0x%08x\n",
                    ...
                    ipc_data->param[0], ipc_data->param[1]);
#endif
            tc_dsp_setCurrAudioBuffer(I2S0_Rx_params, ipc_data->param[1],0);
            {
                void tcc_dma_done_handler(int playback);
                tcc_dma_done_handler(0);
            }
        }
    }
    ...
    if(ipc_data->type == INFO_TYPE){
        if(ipc_data->param[0] == ALSA_OUT_BUF_PARAM){
            ...
            printk("id:0x%08x, type:0x%08x,ALSA_OUT_BUF_PARAM = 0x%08x,CurrAddr 0x%08x\n",
                    ...
                    ipc_data->param[0], ipc_data->param[1]);
#endif
            tc_dsp_setCurrAudioBuffer(I2S1params, ipc_data->param[1],1);

            {
                void tcc_dma1_done_handler(int playback);
                tcc_dma1_done_handler(1);
            }
        }
    }
    ...
    if(ipc_data->type == INFO_TYPE){
        if(ipc_data->param[0] == ALSA_OUT_BUF_PARAM){
            ...
            printk("id:0x%08x, type:0x%08x,ALSA_OUT_BUF_PARAM = 0x%08x,CurrAddr 0x%08x\n",
                    ...
                    ipc_data->param[0], ipc_data->param[1]);
#endif
            tc_dsp_setCurrAudioBuffer(I2S2params, ipc_data->param[1],1);

            {
                void tcc_dma2_done_handler(int playback);
                tcc_dma2_done_handler(1);
            }
        }
    }
