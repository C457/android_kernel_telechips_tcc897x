...
static int kbase_common_device_init(struct kbase_device *kbdev)
{
	...
	dev_info(kbdev->dev,  "Driver version : %s \nProbed as %s\n",
                       MALI_RELEASE_NAME, dev_name(kbdev->mdev.this_device));		//TCC
...
#if 0//defined(CONFIG_OF) && defined(CONFIG_REGULATOR)		//TCC
	kbdev->regulator = regulator_get(kbdev->dev, "vdd_gv");
...
//#endif
	...
	kbdev->clock = clk_get(kbdev->dev, "mali_clk");		//TCC
