...
static struct tcc_gpu_dvfs_table_t gpu_dvfs_table[] = {
       	/* Hz    			uV */
	{ 100000000,   30,   0, 900000 },	
	{ 300000000,   40, 30, 900000 },	
	{ 442500000,   70, 40, 950000 },	
	{ 500000000, 100, 70, 1000000 },	
};
	...
	u32 util_gl_share, u32 util_cl_share[2])
{
	...
	if(currStep != nextStep) {
		if (kbdev->clock) 
		{
			...
			dev_dbg(kbdev->dev, "%s. setting %d, utilisation:%d,  curstep:%d, nextstep:%d, getrate:%d\n", __func__, gpu_dvfs_table[nextStep].gpu_freq, utilisation, currStep, nextStep, clk_get_rate(kbdev->clock));
			return 1;
		}
		else
		{
...
static struct kbase_platform_config TCC8960_platform_config = {
#ifndef CONFIG_OF
...
{
	return &TCC8960_platform_config;
}
