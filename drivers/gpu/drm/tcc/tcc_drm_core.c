					...
					struct tcc_drm_display *display)
{
	...
	ret = tcc_drm_crtc_get_pipe_from_type(dev, display->type);
	if (ret < 0)
		return ret;
	...
	encoder = tcc_drm_encoder_create(dev, display, possible_crtcs);
	if (!encoder) {
...
EXPORT_SYMBOL_GPL(tcc_drm_subdrv_register);
...
EXPORT_SYMBOL_GPL(tcc_drm_subdrv_unregister);
...
EXPORT_SYMBOL_GPL(tcc_drm_device_subdrv_probe);
...
EXPORT_SYMBOL_GPL(tcc_drm_device_subdrv_remove);
...
EXPORT_SYMBOL_GPL(tcc_drm_subdrv_open);
...
EXPORT_SYMBOL_GPL(tcc_drm_subdrv_close);
