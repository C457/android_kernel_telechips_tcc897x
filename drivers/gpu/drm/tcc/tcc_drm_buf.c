...
static int lowlevel_buffer_allocate(struct drm_device *dev,
		unsigned int flags, struct tcc_drm_gem_buf *buf)
{
	int ret = 0;
	enum dma_attr attr;
	 ...
	 */
	if (flags & TCC_BO_WC || !(flags & TCC_BO_CACHABLE))
...
int tcc_drm_alloc_buf(struct drm_device *dev,
		struct tcc_drm_gem_buf *buf, unsigned int flags)
{
...
void tcc_drm_free_buf(struct drm_device *dev,
		unsigned int flags, struct tcc_drm_gem_buf *buffer)
{
