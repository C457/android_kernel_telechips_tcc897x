...
static void update_vm_cache_attr(struct tcc_drm_gem_obj *obj,
					struct vm_area_struct *vma)
{
	DRM_DEBUG_KMS("flags = 0x%x\n", obj->flags);

	/* non-cachable as default. */
	if (obj->flags & TCC_BO_CACHABLE)
		vma->vm_page_prot = vm_get_page_prot(vma->vm_flags);
	else if (obj->flags & TCC_BO_WC)
		vma->vm_page_prot =
...
static int tcc_drm_gem_map_buf(struct drm_gem_object *obj,
					struct vm_area_struct *vma,
					unsigned long f_vaddr,
					pgoff_t page_offset)
{
	struct tcc_drm_gem_obj *tcc_gem_obj = to_tcc_gem_obj(obj);
	struct tcc_drm_gem_buf *buf = tcc_gem_obj->buffer;
...
void tcc_drm_gem_destroy(struct tcc_drm_gem_obj *tcc_gem_obj)
{
	...
	obj = &tcc_gem_obj->base;
	buf = tcc_gem_obj->buffer;
	...
	tcc_drm_free_buf(obj->dev, tcc_gem_obj->flags, buf);

out:
	tcc_drm_fini_buf(obj->dev, buf);
	tcc_gem_obj->buffer = NULL;
						...
						struct drm_file *file_priv)
{
	...
	return tcc_gem_obj->buffer->size;
}
						      ...
						      unsigned long size)
{
	...
	tcc_gem_obj = kzalloc(sizeof(*tcc_gem_obj), GFP_KERNEL);
	if (!tcc_gem_obj)
		return NULL;

	tcc_gem_obj->size = size;
	obj = &tcc_gem_obj->base;
						...
						unsigned long size)
{
	...
	buf = tcc_drm_init_buf(dev, size);
	if (!buf)
		return ERR_PTR(-ENOMEM);

	tcc_gem_obj = tcc_drm_gem_init(dev, size);
	if (!tcc_gem_obj) {
		ret = -ENOMEM;
		goto err_fini_buf;
	}

	tcc_gem_obj->buffer = buf;

	/* set memory type and cache attribute from user side. */
	tcc_gem_obj->flags = flags;

	ret = tcc_drm_alloc_buf(dev, buf, flags);
	if (ret < 0)
		goto err_gem_fini;
...
err_gem_fini:
	drm_gem_object_release(&tcc_gem_obj->base);
	kfree(tcc_gem_obj);
err_fini_buf:
	tcc_drm_fini_buf(dev, buf);
	return ERR_PTR(ret);
}
				...
				struct drm_file *file_priv)
{
	...
	tcc_gem_obj = tcc_drm_gem_create(dev, args->flags, args->size);
	if (IS_ERR(tcc_gem_obj))
		return PTR_ERR(tcc_gem_obj);

	ret = tcc_drm_gem_handle_create(&tcc_gem_obj->base, file_priv,
			&args->handle);
	if (ret) {
					...
					struct drm_file *filp)
{
	...
	return &tcc_gem_obj->buffer->dma_addr;
}
...
int tcc_drm_gem_mmap_buffer(struct tcc_drm_gem_obj *tcc_gem_obj,
				      struct vm_area_struct *vma)
{
	struct drm_device *drm_dev = tcc_gem_obj->base.dev;
...
{	struct tcc_drm_gem_obj *tcc_gem_obj;
	...
	args->flags = tcc_gem_obj->flags;
	args->size = tcc_gem_obj->size;
...
void tcc_drm_gem_free_object(struct drm_gem_object *obj)
{
	...
	tcc_drm_gem_destroy(to_tcc_gem_obj(obj));
}
			       ...
			       struct drm_mode_create_dumb *args)
{
	...
	args->pitch = ALIGN(args->pitch, 8);  // _egl_memory_create_buffer() line:331  pitch should be 8 aligned.
	args->size = PAGE_ALIGN(args->pitch * args->height);

	if (is_drm_iommu_supported(dev)) {
		tcc_gem_obj = tcc_drm_gem_create(dev,
			TCC_BO_NONCONTIG | TCC_BO_WC,
			args->size);
	} else {
		tcc_gem_obj = tcc_drm_gem_create(dev,
			TCC_BO_CONTIG | TCC_BO_WC,
			args->size);
	}
...
int tcc_drm_gem_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
	...
	ret = tcc_drm_gem_map_buf(obj, vma, f_vaddr, page_offset);
	if (ret < 0)
...
int tcc_drm_gem_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	ret = check_gem_flags(tcc_gem_obj->flags);
	if (ret)
		goto err_close_vm;

	update_vm_cache_attr(tcc_gem_obj, vma);

	ret = tcc_drm_gem_mmap_buffer(tcc_gem_obj, vma);
	if (ret)
		goto err_close_vm;
