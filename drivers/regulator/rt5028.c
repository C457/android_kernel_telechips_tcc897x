...
static int rt5028_pmic_probe(struct i2c_client *client, const struct i2c_device_id *i2c_id)
{
	...
	if (gpio_is_valid(irq_port))
	{
		...
		printk("\x1b[1;33m[%s:irq=%d]\x1b[0m\n", __func__, client->irq);	
	}
	...
	printk("\x1b[1;33m[%s]pmic id = 0x%x\x1b[0m\n", __func__, i2c_data);
...
//	}
	...
	for_each_child_of_node(regulators_np, reg_np) {
		...
		id = rt5028_reg[i].id;
		config.init_data = of_get_regulator_init_data(&client->dev, reg_np);
	...
	if (client->irq) {
		...
		if (request_irq(client->irq, rt5028_interrupt, IRQ_TYPE_EDGE_FALLING|IRQF_DISABLED, "rt5028_irq", rt5028)) {
...
static int rt5028_pmic_suspend(struct i2c_client *client, pm_message_t mesg)
{
	int i, ret = 0;
	struct regulator_dev **rdev = i2c_get_clientdata(client);
	struct rt5028_data* rt5028 = NULL;
...
static int rt5028_pmic_resume(struct i2c_client *client)
{
	int i;
	struct regulator_dev **rdev = i2c_get_clientdata(client);
	struct rt5028_data* rt5028 = NULL;
...
static struct i2c_driver rt5028_pmic_driver = {
	...
	.suspend = rt5028_pmic_suspend,
	.resume	 = rt5028_pmic_resume,
	.driver	 = {
	.name    = "rt5028",
#ifdef CONFIG_OF
