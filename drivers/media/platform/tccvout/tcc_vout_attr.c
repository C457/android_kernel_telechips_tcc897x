...
static ssize_t deinterlace_path_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	case VOUT_DEINTL_S:
		ret = sprintf(buf, "RDMA%d - DEINTL_S - SC%d - WMIX%d - WDMA%d\n",
			vioc->deintl_rdma.id, vioc->deintl_sc.id, vioc->deintl_wmix.id, vioc->deintl_wdma.id);
		break;
	case VOUT_DEINTL_VIQE_2D:
		ret = sprintf(buf, "RDMA%d - VIQE_2D - SC%d - WMIX%d - WDMA%d\n",
			vioc->deintl_rdma.id, vioc->deintl_sc.id, vioc->deintl_wmix.id, vioc->deintl_wdma.id);
		break;
	case VOUT_DEINTL_VIQE_3D:
		ret = sprintf(buf, "RDMA%d - VIQE_3D - SC%d - WMIX%d - WDMA%d\n",
			vioc->deintl_rdma.id, vioc->deintl_sc.id, vioc->deintl_wmix.id, vioc->deintl_wdma.id);
		break;
	case VOUT_DEINTL_VIQE_BYPASS:
		ret = sprintf(buf, "RDMA%d - VIQE_BYPASS - SC%d - WMIX%d - WDMA%d\n",
			vioc->deintl_rdma.id, vioc->deintl_sc.id, vioc->deintl_wmix.id, vioc->deintl_wdma.id);
		break;
	case VOUT_DEINTL_NONE:
	default:
		ret = sprintf(buf, "RDMA%d - SC%d - WMIX%d - WDMA%d\n",
			vioc->deintl_rdma.id, vioc->deintl_sc.id, vioc->deintl_wmix.id, vioc->deintl_wdma.id);
		break;
	}
...
static ssize_t deinterlace_rdma_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dprintk("set(%d) -> deintl_rdma(%d)\n", vioc->deintl_rdma.id, val);
...
static ssize_t deinterlace_sc_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dprintk("deintl_sc%d -> deintl_sc%d\n", vioc->deintl_sc.id, val);
...
static ssize_t deinterlace_bfield_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dprintk("set(%d) -> deintl_rdma.bf(%d)\n", vioc->deintl_rdma.bf, val);
	vioc->deintl_rdma.bf = val;
	return count;
}
...
void tcc_vout_attr_create(struct platform_device *pdev)
{
	...
	device_create_file(&pdev->dev, &dev_attr_vioc_sc);
	#endif
}
...
void tcc_vout_attr_remove(struct platform_device *pdev)
{
	...
	#ifdef VOUT_DISP_PATH_SCALER
	device_remove_file(&pdev->dev, &dev_attr_vioc_sc);
	#endif
}
