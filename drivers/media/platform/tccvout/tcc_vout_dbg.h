...
static inline void kfree_safe(const void *x)
{
	struct page *page;
	...
	if (unlikely(((unsigned int)&(page->flags) == 0xffffffff))) {
		printk("%s: page == 0xffffffff\n", __func__);
		goto exit;
	}

	if (unlikely(!PageSlab(page))) {
		if (!PageCompound(page)) {
			printk("%s: PageCompound\n", __func__);
			goto exit;
		}
	}
#endif
...
extern void print_v4l2_rect(struct v4l2_rect *rect, const char *str);
