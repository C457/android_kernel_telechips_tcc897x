...
static int ths8200_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	of_property_read_u32(np, "reg", (u32 *)&client->addr);
 	...
 	if(np) 
 	{
		...
		{
			printk("%s, field port: 0x%02x\n", __func__, ths8200_field_port);
			gpio_request(ths8200_field_port, "ths8200-field");
		}
		else
		{
			printk("%s, err to get field-gpios\n", __func__);
...
void ths8200_reset(void)
{
	...
	/* set field port */
	tcc_gpio_config(ths8200_field_port, GPIO_FN0|GPIO_OUTPUT|GPIO_HIGH);
