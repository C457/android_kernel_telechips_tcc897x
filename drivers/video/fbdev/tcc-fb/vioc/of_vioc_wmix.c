...
#define MIXER_LAYER_OFFSET	0x20
...
void vioc_wmix_set_image(struct vioc_wmix_device *wmix, unsigned int en)
{
	...
	if (wmix->data.layer < 4) {
		if (wmix->data.region<4) {
			...
			__raw_writel(value, wmix->reg + MACON + wmix->data.layer*MIXER_LAYER_OFFSET);

			/* color control */
			value = ((wmix->data.ccon1&0xF)<<4|(wmix->data.ccon0&0xF))<<(wmix->data.region*8);
			__raw_writel(value, wmix->reg + MCCON + wmix->data.layer*MIXER_LAYER_OFFSET);
		}

		/* ROP control */
		//value = __raw_readl(wmix->reg + MROPC0 + wmix->data.layer*MIXER_LAYER_OFFSET) & ~(0x3<<14 | 0x1F);
		value |= ((wmix->data.asel&0x3) << 14) | (wmix->data.mode&0x1F);
		__raw_writel(value, wmix->reg + MROPC0 + wmix->data.layer*MIXER_LAYER_OFFSET);

		//value = __raw_readl(wmix->reg + MROPC1 + wmix->data.layer*MIXER_LAYER_OFFSET) & ~(0xFFFFFFFF);
		value |= ((wmix->data.alpha1&0xFFFF)<<16) | wmix->data.alpha0&0xFFFF;
		__raw_writel(value, wmix->reg + MROPC1 + wmix->data.layer*MIXER_LAYER_OFFSET);
