...
VIOC_CONFIG_PATH_u *VIOC_CONFIG_GetPathStruct (unsigned int nType)
{
	...
	switch(nType) {
		...
		case (VIOC_SC3):
			...
			break;
		case (VIOC_SC4):
			pConfigPath = (VIOC_CONFIG_PATH_u *)&pIREQConfig->uSC4.nREG;
			break;
		case (VIOC_VIQE):
			pConfigPath = (VIOC_CONFIG_PATH_u *)&pIREQConfig->uVIQE.nREG;
			break;
		case (VIOC_VIQE1):
			pConfigPath = (VIOC_CONFIG_PATH_u *)&pIREQConfig->uVIQE1.nREG;
			break;
		case (VIOC_DEINTLS):
...
void VIOC_CONFIG_SWReset(VIOC_IREQ_CONFIG* pVIOCConfig, unsigned int vioc,unsigned int vioc_index,unsigned int Mode)
{
	if(Mode == VIOC_CONFIG_RESET)
	{
		switch(vioc)
		{
			...
			case VIOC_CONFIG_SCALER:
				BITCSET(pVIOCConfig->uSOFTRESET2.nREG, (1<<(vioc_index+16)), (1<<(vioc_index+16)));	// PWR_BLK_SWR2 register
				break;
	...
	else if(Mode == VIOC_CONFIG_CLEAR)
	{
		switch(vioc)
		{
			...
			case VIOC_CONFIG_SCALER:
				BITCSET(pVIOCConfig->uSOFTRESET2.nREG, (1<<(vioc_index+16)), (0<<(vioc_index+16)));	// PWR_BLK_SWR2 register
				break;
...
int tca_get_RDMA_PlugIn_to_Scaler(unsigned int RdmaN)
{
	...
	for(i =0; i <= VIOC_SC4; i++)
	{
		 VIOC_CONFIG_Device_PlugState(i, &VIOC_PlugIn);
		if(VIOC_PlugIn.enable && VIOC_PlugIn.connect_device == tca_SC_PlugIn_RdmaN(RdmaN))
			return i;
	}
	return -1;
}
...
int tca_set_MC_Connect_to_RDMA(unsigned int McN, unsigned int RdmaN, unsigned int SetClr)
{
	...
	{
		case 3:
			ret  =  BITCSET(pIREQConfig->uMC.nREG, (0x1<< 0) | (0x7<< map_shift), (SetClr << 0) | (0x0<< map_shift));
			break;

		case 7:
			BITCSET(pIREQConfig->uMC.nREG,  (0x1<< 1) | (0x7<< map_shift), (SetClr << 1) | (0x1<< map_shift));
			break;

		case 11:
			BITCSET(pIREQConfig->uMC.nREG,  (0x1<< 2) | (0x7<< map_shift) , (SetClr << 2) | (0x2<< map_shift));
			break;

		case 13:
			BITCSET(pIREQConfig->uMC.nREG,  (0x1<< 3) | (0x7<< map_shift), (SetClr << 3) | (0x3<< map_shift));

			break;			
		case 15:
			BITCSET(pIREQConfig->uMC.nREG,  (0x1<< 4) | (0x7<< map_shift), (SetClr << 4) | (0x4<< map_shift));
			break;			
		case 16:
			BITCSET(pIREQConfig->uMC.nREG,  (0x1<< 5) | (0x7<< map_shift), (SetClr << 5) | (0x5<< map_shift));
			break;			
		case 17:
			BITCSET(pIREQConfig->uMC.nREG,  (0x1<< 6) | (0x7<< map_shift), (SetClr << 6) | (0x6<< map_shift));
			break;
