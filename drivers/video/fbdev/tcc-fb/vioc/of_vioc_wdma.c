...
#define WDMABG1		0x28
#define	WDMAPTS		0x2C
#define WDMADMAT0	0x30
#define WDMADMAT1	0x34
#define WDMAENH		0x38
#define WDMAROLL	0x3C
#define WDMASBASE	0x40
#define WDMAIRQSTS	0x44
#define WDMAIRQMSK	0x48
...
void vioc_wdma_set_image(struct vioc_wdma_device *wdma, unsigned int en)
{
	...
	/* mode */
	value = __raw_readl(wdma->reg + WDMACTRL) & ~((7<<18)|(1<<17) | (7<<12) | (7<<9)|(1<<8));
	value |= (wdma->data.mode.rgb_swap&0x7)<<12;	/* rgb swap mode */
	value |= (wdma->data.mode.convert&0x7)<<18 | (wdma->data.mode.convert&0x7)<<9;	/* converter mode */
