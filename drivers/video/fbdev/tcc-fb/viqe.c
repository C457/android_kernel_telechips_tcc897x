...
viqe_queue_entry_t *
viqe_queue_get_entry_to_push (viqe_queue_t *p_queue)
{
	int new_idx;
...
void
viqe_render_frame (struct tcc_lcdc_image_update *input_image, unsigned int field_interval, int curTime,int reset_frmCnt)							
{
	...
	p_entry = viqe_queue_get_entry_to_push(&g_viqe_render_queue);	//to insert first field
	p_entry_image = &p_entry->input_image;
	memcpy(p_entry_image,input_image,sizeof(struct tcc_lcdc_image_update));
	p_entry_image->odd_first_flag= (input_image->odd_first_flag) ? 1 : 0;
	p_entry->reset_frmCnt = reset_frmCnt;
	p_entry->new_field		= 1;
	
	
	p_entry = viqe_queue_get_entry_to_push(&g_viqe_render_queue);	//to insert second field
	p_entry_image = &p_entry->input_image;
	memcpy(p_entry_image,input_image,sizeof(struct tcc_lcdc_image_update));
...
//		printk ("[REND] queue is full ...\n");
	...
	if ( !g_viqe_render_queue.ready ) {
		//viqe_render_field (input_image, curTime);
...
//	sim_value (g_viqe_render_queue.inout_rate);
	...
	if ( g_viqe_render_queue.ready ) {
		...
		if ( g_viqe_render_queue.frame_cnt < 4 ) {
			p_entry->input_image.odd_first_flag = g_viqe_render_queue.frame_cnt&0x1;
		}
		...
		TCC_VIQE_DI_Run60Hz(1, p_entry->frame_base0, p_entry->frame_base1, p_entry->frame_base2,
							...
							p_entry->offset_x, p_entry->offset_y, p_entry->bottom_field, p_entry->frameInfo_interlace, p_entry->reset_frmCnt);
		#else
