...
static sDRV_UART_SC_ATR gDRV_UART_SC_ATR = {0, TCC_NSK_SC_DIRECT_CONVENTION, 0, 0, {0, }, {0, }, {0, }, {0,}, {0, }, 0};
...
static sDRV_UART_DMA_REG gDRV_UART_DMA_REG = 
{
	...
	, 0
	, 0
	|DRV_DMA_CHCTRL_EnableCont			
	|DRV_DMA_CHCTRL_EnableSync			
	|DRV_DMA_CHCTRL_EnableHRDWR			
	|DRV_DMA_CHCTRL_EnableLock			
	|DRV_DMA_CHCTRL_BSize_1				
	|DRV_DMA_CHCTRL_WSize_8				
	|DRV_DMA_CHCTRL_EnableRepeatMode		
};

static sDRV_UART_DMA_BUF gDRV_UART_DMA_BUF = {0, 0, NULL, NULL, DRV_UART_SC_RX_DMA_BUF_SIZE};

static sDRV_UART_IOBUS_REG gDRV_UART_IOBUS_REG =
{
	NULL
	, 0
	, 0
...
void tca_nsk_sc_rx_print(void)
{
	...
	for(i=0; i<100; i+=10)
	{
		printk("%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X\n", \
				tca_nsk_sc_ts_pattern_check(p[i]), tca_nsk_sc_ts_pattern_check(p[i+1]), tca_nsk_sc_ts_pattern_check(p[i+2]), tca_nsk_sc_ts_pattern_check(p[i+3]), 
				tca_nsk_sc_ts_pattern_check(p[i+4]), tca_nsk_sc_ts_pattern_check(p[i+5]), tca_nsk_sc_ts_pattern_check(p[i+6]), tca_nsk_sc_ts_pattern_check(p[i+7]),  
				tca_nsk_sc_ts_pattern_check(p[i+8]), tca_nsk_sc_ts_pattern_check(p[i+9]));
	}
...
//---------------------------------------------------------------------------
static void tca_nsk_sc_calc_factor_T0(unsigned short usTA1, unsigned short usTC1, unsigned short usTC2)
{
	sDRV_UART_SC_CLK *pstClk = &gDRV_UART_SC_CLK;
...
static void tca_nsk_sc_calc_factor_T1(unsigned short usTA1, unsigned short usTC1, unsigned short usTB3)
{
	sDRV_UART_SC_CLK *pstClk = &gDRV_UART_SC_CLK;
	unsigned uHighNibble, uLowNibble;
	int iCWI=1, iBWI=1;
...
int tca_nsk_sc_send_receive(char *pTxBuf, unsigned uTxBufLen, char *pRxBuf, unsigned *puRxBufLen, unsigned uDirection, unsigned uTimeOut)
{
	//sDRV_UART_SC_CLK *pstClk = &gDRV_UART_SC_CLK;
	...
	if(uDirection == TCC_NSK_SC_DIRECTION_TO_CARD) {
		...
		if(iRet == TCC_NSK_SC_ERROR_COMPLEMENT) {
			for(i=0; i<iDataLen; i++)
			{
				...
				if(iRet) {
					//eprintk("%s: timeout Send-1(%d, %d, %d)\n", __func__, pstClk->WT, uTimeOut, *puRxBufLen);
					return TCC_NSK_SC_ERROR_TIMEOUT;
				}
			...
			if(iRet) {
				//eprintk("%s: timeout Send-2(%d, %d, %d)\n", __func__, pstClk->WT, uTimeOut, *puRxBufLen);
				return TCC_NSK_SC_ERROR_TIMEOUT;
			}
		}
...
int tca_nsk_sc_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(pdev->dev.of_node, "iobus_dmareqsel_offset", &pstIOBUSReg->DMAREQSELOffset);
	...
	pstDMABuf->pVRxBuf = dma_alloc_coherent(0, pstDMABuf->uiRxBufSize, &pstDMABuf->pPRxBuf, GFP_KERNEL);
...
#if 1
	dprintk("%s(%d)\n", __func__, giSCIrq);
	dprintk("%s(%d, %d, %d, %d)\n", __func__, pstClk->Fuart, pstReg->CH, pstPort->PortNum, pstPort->TDA8024);
