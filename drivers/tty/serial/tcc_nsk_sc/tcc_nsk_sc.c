...
static int tcc_nsk_sc_select_vcc_level(unsigned uVccLevel)
{
	...
	if(uVccLevel != TCC_NSK_SC_VOLTAGE_LEVEL_3V \
		&& uVccLevel != TCC_NSK_SC_VOLTAGE_LEVEL_5V) {
...
static int tcc_nsk_sc_send_receive(stTCC_NSK_SC_BUF stSCBuf)
{
	...
	iRet = tca_nsk_sc_send_receive(stSCBuf.pucTxBuf, stSCBuf.uiTxBufLen, stSCBuf.pucRxBuf, stSCBuf.puiRxBufLen, stSCBuf.uiDirection, stSCBuf.uiTimeOut);
	if(iRet) {
		eprintk("%s(%d)\n", __func__, iRet);
		return iRet;
	}
