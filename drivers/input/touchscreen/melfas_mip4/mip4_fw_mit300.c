...
* Read ISC status
*/
static int mip4_ts_isc_read_status(struct mip4_ts_info *info)
{
...
* Command : Erase Page
*/
static int mip4_ts_isc_erase_page(struct mip4_ts_info *info, int offset)
{
	...
	if(mip4_ts_isc_read_status(info) != 0){
		goto ERROR;
	}
...
* Command : Read Page
*/
static int mip4_ts_isc_read_page(struct mip4_ts_info *info, int offset, u8 *data, int length)
{
...
*/
static int mip4_ts_isc_program_page(struct mip4_ts_info *info, int offset, const u8 *data, int length)
{
...
* Command : Enter ISC
*/
static int mip4_ts_isc_enter(struct mip4_ts_info *info)
{
...
* Command : Exit ISC
*/
static int mip4_ts_isc_exit(struct mip4_ts_info *info)
{
...
int mip4_ts_flash_fw(struct mip4_ts_info *info, const u8 *fw_data, size_t fw_size, bool force, bool section)
{
	...
	ret = mip4_ts_isc_enter(info);
	if(ret != 0){
		dev_err(&client->dev,"%s [ERROR] mip4_ts_isc_enter\n", __func__);
		ret = fw_err_download;
		goto ERROR_UPDATE;
	}
	...
	ret = mip4_ts_isc_erase_page(info, offset_start);
	if(ret != 0){
		dev_err(&client->dev,"%s [ERROR] mip4_ts_isc_erase_page\n", __func__);
		ret = fw_err_download;
		goto ERROR_UPDATE;
	}
	...
	while(offset >= offset_start){
		//Program
		if(mip4_ts_isc_program_page(info, offset, &bin_data[offset], ISC_PAGE_SIZE)){
			dev_err(&client->dev, "%s [ERROR] mip4_ts_isc_program_page : offset[0x%04X]\n", __func__, offset);
			ret = fw_err_download;
			goto ERROR_UPDATE;
		}
		dev_dbg(&client->dev, "%s - mip4_ts_isc_program_page : offset[0x%04X]\n", __func__, offset);

		//Verify
		if(mip4_ts_isc_read_page(info, offset, rbuf, 4)){
			dev_err(&client->dev, "%s [ERROR] mip4_ts_isc_read_page : offset[0x%04X]\n", __func__, offset);
			ret = fw_err_download;
			goto ERROR_UPDATE;
		}
		dev_dbg(&client->dev, "%s - mip4_ts_isc_read_page : offset[0x%04X]\n", __func__, offset);

#if FW_UPDATE_DEBUG
		print_hex_dump(KERN_ERR, MIP4_TS_DEVICE_NAME " F/W File : ", DUMP_PREFIX_OFFSET, 16, 1, &bin_data[offset], 4, false);
		print_hex_dump(KERN_ERR, MIP4_TS_DEVICE_NAME " F/W Chip : ", DUMP_PREFIX_OFFSET, 16, 1, rbuf, 4, false);
#endif
