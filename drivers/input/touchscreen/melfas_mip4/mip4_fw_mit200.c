...
* Command : Read Page
*/
static int isc_read_page(struct mip4_ts_info *info, int offset, u8 *data, int length)
{
...
*/
static int isc_program_page(struct mip4_ts_info *info, int offset, const u8 *data, int length)
{
...
static int isc_erase_page(struct mip4_ts_info *info, int offset)
{
	...
	ret = isc_program_page(info, offset, data, ISC_PAGE_SIZE);
	if (ret != 0) {
		dev_err(&info->client->dev,"%s [ERROR] isc_program_page\n", __func__);
		goto error;
	}
...
int mip4_ts_flash_fw(struct mip4_ts_info *info, const u8 *fw_data, size_t fw_size, bool force, bool section)
{
	...
	while (offset >= offset_start) {
		//Program
		if (isc_program_page(info, offset, &bin_data[offset], ISC_PAGE_SIZE)) {
			dev_err(&client->dev, "%s [ERROR] isc_program_page : offset[0x%04X]\n", __func__, offset);
			ret = fw_err_download;
			goto error_update;
		}
		dev_dbg(&client->dev, "%s - isc_program_page : offset[0x%04X]\n", __func__, offset);

		//Verify
		if (isc_read_page(info, offset, rbuf, ISC_PAGE_SIZE)) {
