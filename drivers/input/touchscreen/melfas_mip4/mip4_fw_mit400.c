...
int mip4_ts_flash_fw(struct mip4_ts_info *info, const u8 *fw_data, size_t fw_size, bool force, bool section)
{
	...
	if ((bin_info->ver_boot != 0xEEEE) && (bin_info->ver_boot != 0xFFFF) && (bin_info->ver_core == 0xEEEE) && (bin_info->ver_app == 0xEEEE) && (bin_info->ver_param == 0xEEEE)) {
		...
		dev_err(&client->dev, "%s [ERROR] wrong file type\n", __func__);
		ret = fw_err_file_type;
		goto error_file;
	}
	...
	for (flash_addr = flash_addr_start; flash_addr < flash_addr_end; flash_addr += BL_PAGE_SIZE) {
		...
		if (bl_write_page(info, flash_addr, &bin_data[bin_addr], BL_PAGE_SIZE, buf_addr)) {
		...
		if (bl_read_page(info, flash_addr, rbuf, BL_PAGE_SIZE, buf_addr)) {
...
#if FW_UPDATE_DEBUG
		print_hex_dump(KERN_ERR, MIP4_TS_DEVICE_NAME " F/W File : ", DUMP_PREFIX_OFFSET, 16, 1, &bin_data[bin_addr], BL_PAGE_SIZE, false);
		print_hex_dump(KERN_ERR, MIP4_TS_DEVICE_NAME " F/W Chip : ", DUMP_PREFIX_OFFSET, 16, 1, rbuf, BL_PAGE_SIZE, false);
#endif

		if (memcmp(rbuf, &bin_data[bin_addr], BL_PAGE_SIZE)) {
