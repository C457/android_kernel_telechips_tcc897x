 ...
 *
 *
 * mip4_fw_mms500.c : Firmware update functions for MMS500
 *
 * Version : 2016.11.30
 */
...
* Command : Erase Page
*/
static int __maybe_unused isc_erase_page(struct mip4_ts_info *info, int offset)
{
...
int mip4_ts_restore_fw(struct mip4_ts_info *info, const u8 *fw_data, size_t fw_size, u8 *sections)
{
	...
	dev_dbg(&client->dev, "%s - bin_info : bin_len[%d] chip_name[%s] hw_cat[0x%02X] date[%04X] time[%04X] tail_size[%d]\n", __func__, bin_info->bin_length, bin_info->chip_name, bin_info->hw_category, bin_info->build_date, bin_info->build_time, bin_info->tail_size);
	...
	if (memcmp(bin_info->chip_name, FW_CHIP_CODE, 4)) {
		dev_err(&client->dev, "%s [ERROR] F/W file is not for %s (%s)\n", __func__, CHIP_NAME, FW_CHIP_CODE);
		ret = fw_err_file_type;
		goto ERROR_FILE;
	} 
	...
	image_data = kzalloc((sizeof(u8) * flash_size), GFP_KERNEL);
	...
	mass_erase = false;
	dev_dbg(&client->dev, "%s - offset_start[0x%04X] offset_end[0x%04X] mass_erase[%d]\n", __func__, offset_start, offset_end, mass_erase);
	...
	if (isc_read_cal_data(info, rbuf)) {
...
#if FW_UPDATE_DEBUG
	print_hex_dump(KERN_ERR, MIP4_TS_DEVICE_NAME " Image : ", DUMP_PREFIX_OFFSET, 16, 1, image_data, flash_size, false);
#endif
	...
	if (mass_erase == true) {
		...
		dev_dbg(&client->dev, "%s - isc_erase_mass\n", __func__);
	}
	...
	while (offset >= offset_start) {
		//Section
		i_section = offset / 1024;

		if (sections[i_section] == 0) {
			dev_dbg(&client->dev, "%s - offset[0x%04X] page[%d] i_section[%d] : skip\n", __func__, offset, offset / ISC_PAGE_SIZE, i_section);

			offset -= ISC_PAGE_SIZE;
			continue;
		}
		dev_dbg(&client->dev, "%s - offset[0x%04X] page[%d] i_section[%d] : restore\n", __func__, offset, offset / ISC_PAGE_SIZE, i_section);

		if (prev_section != i_section) {
			section_cnt++;

			if (section_cnt > MAX_RESTORE_SECTION_NUM) {
				dev_dbg(&client->dev, "%s - section_cnt[%d] max_sections[%d]\n", __func__, section_cnt, MAX_RESTORE_SECTION_NUM);
				break;
			}
		}
		dev_dbg(&client->dev, "%s - section_cnt[%d]\n", __func__, section_cnt);
		prev_section = i_section;

		if (mass_erase == false) {
			//Erase page
			ret = isc_erase_page(info, offset);
			if (ret != 0) {
...
int mip4_ts_flash_fw(struct mip4_ts_info *info, const u8 *fw_data, size_t fw_size, bool force, bool section)
{
	...
	//Check chip code
	if (memcmp(bin_info->chip_name, FW_CHIP_CODE, 4)) {
		dev_err(&client->dev, "%s [ERROR] F/W file is not for %s : chip_name[%s]\n", __func__, CHIP_NAME, FW_CHIP_CODE);
		ret = fw_err_file_type;
		goto ERROR_FILE;
	}
	...
	if (force == true) {
		...
		if (!mip4_ts_get_fw_version_u16(info, ver_chip)) {
			...
			if ((ver_chip[0] == bin_info->ver_boot) && (ver_chip[1] == bin_info->ver_core) && (ver_chip[2] == bin_info->ver_app) && (ver_chip[3] == bin_info->ver_param)) {
				...
				dev_info(&client->dev, "%s - Firmware version mismatch.\n", __func__);
				offset_start = 0;
			}
		} else {
