...
void siw_hal_set_gpio_reset(struct device *dev, int val)
{
	...
	if(!get_montype())
	{
		...
		t_dev_info(dev, "%s - CE pin by I2C : %d\n",__func__,!!(val));
...
/*
 * [New] debugging initial frame count
 */
static int siw_hal_dbg_frame_count(struct device *dev)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	u32 frame_addr = 0;
	u32 frame_count = ~0;
	...
	siw_hal_read_value(dev, frame_addr, &frame_count);

	t_dev_info(dev, "chip frame count %d\n", frame_count);
...
static int siw_hal_do_ic_info(struct device *dev, int prt_on)
{
	...
	/*
	* [New] read frame count in initial stage
	*/
	siw_hal_dbg_frame_count(dev);
...
/* usage
 * (1) echo <value> > /sys/module/{Siw Touch Module Name}/parameters/alive_dbg_mask
...
int siw_hal_alive_level_get(struct device *dev)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);

	return chip->alive_level;
}

static int __siw_hal_alive_level_set(struct device *dev, int new_level, int set)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct siw_ts *ts = chip->ts;
	u32 addr = 0;
	int min_level = ALIVE_LEVEL_MIN;
	int max_level = ALIVE_LEVEL_MAX;
	int cur_level = 0;
	int update = 0;
	int ret = 0;

	if (new_level == -1) {
		new_level = ALIVE_LEVEL_DEFAULT;
	}
	...
	if ((cur_level < min_level) || (cur_level > max_level)) {
		t_dev_warn(dev, "alive not supported\n");
		return 0;
	}
	...
	update = (new_level != cur_level);

	if (update) {
		/* set new level */
		ret = siw_hal_write_value(dev, addr, new_level);
		if (ret < 0) {
			return ret;
		}
	}

	chip->alive_level = new_level;

	t_dev_info(dev, "alive level: %s%s%s\n",
		siw_alive_level_str(new_level),
		(set && update) ? " <- " : "",
		(set && update) ? siw_alive_level_str(cur_level) : "");

	return 1;
}

int __used siw_hal_alive_level_set(struct device *dev, int new_level)
{
	return __siw_hal_alive_level_set(dev, new_level, 1);
}

void __used siw_hal_irq_count_clr(struct device *dev, int sel)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	...
	t_dev_info(dev, "reset irq_count\n");
}

static int __used siw_hal_irq_mon_init(struct device *dev)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct siw_ts *ts = chip->ts;
	int rst_level = siw_hal_alive_level_get(dev);

	if (atomic_read(&ts->state.core) == CORE_PROBE) {
		rst_level = ALIVE_LEVEL_DEFAULT;
	}

	switch (touch_chip_type(ts)) {
	case CHIP_SW17700:
		__siw_hal_alive_level_set(dev, rst_level, 0);
		break;
	}
...
static int __used __irq_mon_for_others(struct device *dev)
{
	...
	t_alive_dbg_trace(dev, "irq_count_others: %d\n", count);
...
/*
 * Base wrapper function for alive detection
 */
static int __used siw_hal_irq_mon_check(struct device *dev)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct siw_ts *ts = chip->ts;
	u32 status = 0;
	...
	status = chip->info.device_status;

	if (!(status & chip->status_mask_bit.valid_irq_pin)) {
		return 0;
	}

	irq_type = (status>>16) & 0xF;
...
/*
 * New sub-function for reset control
 */
