...
void VIOC_VIQE_InitDeintlCoreVinMode(VIQE *pVIQE)
{
    BITCSET(pVIQE->cDEINTL.nDI_ENGINE3, 0xFFFFFFFF, 0x00202000);
}

void VIOC_VIQE_SetY2R(VIQE *pVIQE, unsigned int uiEnableY2R)
{
    unsigned int uiMask = Hw8;
    unsigned int uiValue = (uiEnableY2R)?Hw8:0;

    BITCSET(pVIQE->cVIQE_CTRL.nVIQE_CTRL_Y2R.nREG, uiMask, uiValue);
}

void VIOC_VIQE_DUMP(VIQE *pVIQE)
{
	unsigned int cnt = 0;
	char *pReg = (char*)pVIQE;
