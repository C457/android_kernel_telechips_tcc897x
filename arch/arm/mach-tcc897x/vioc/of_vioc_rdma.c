...
#define RDMABG		0x24
#define	RDMAPTS		0x28
#define RDMADMAT0	0x2C
#define RDMADMAT1	0x30
#define RDMAENH		0x34
#define RDMAROLL	0x38
#define RDMASBASE	0x3C
#define RDMAIRQSTS	0x40
#define RDMAIRQMSK	0x44
...
void vioc_rdma_set_image(struct vioc_rdma_device *rdma, unsigned int en)
{
	...
	/* mode */
	value = __raw_readl(rdma->reg + RDMACTRL) & ~((3<<18)|(1<<17) | (7<<12) | (3<<9)|(1<<8));
	value |= (rdma->data.mode.rgb_swap&0x7)<<12;	/* rgb swap mode */
	value |= (rdma->data.mode.convert&0x3)<<18 | (rdma->data.mode.convert&0x3)<<9;	/* converter mode */
