									...
									unsigned int start_x, unsigned int start_y, unsigned int* Y, unsigned int* U,unsigned int* V)
{
	...
	if((format >= TCC_LCDC_IMG_FMT_RGB332) && (format <= TCC_LCDC_IMG_FMT_ARGB6666_3))
	{
		...
		/* TCC_LCDC_IMG_FMT_RGB332 = 1 bpp
		 * TCC_LCDC_IMG_FMT_RGB444 ~ TCC_LCDC_IMG_FMT_RGB555 = 2 bpp
		 * TCC_LCDC_IMG_FMT_RGB888 ~ TCC_LCDC_IMG_FMT_ARGB6666_3 = 4 bpp
		 */
		if (format == TCC_LCDC_IMG_FMT_RGB332)
			Bpp = 1;
		else if (format <= TCC_LCDC_IMG_FMT_RGB555)
			Bpp = 2;
		else
			Bpp = 4;
...
unsigned int tcc_vioc_display_dt_parse(struct device_node *np, 	struct tcc_dp_device *dp_data)
{
	...
	dp_data->ddc_info.virt_addr = of_iomap(ddc_node, 0);

	if (of_property_read_u32(ddc_node, "disp_number", &dp_data->ddc_info.blk_num)){
		pr_err( "could not find ddc nubmer\n");
		ret = -ENODEV;
	}
	dp_data->DispNum = dp_data->ddc_info.blk_num;
	dp_data->ddc_info.irq_num = irq_of_parse_and_map(ddc_node, 0);
	
	dp_data->ddc_clock = of_clk_get(ddc_node, 0);
	BUG_ON(dp_data->ddc_clock == NULL);

	dp_data->vioc_clock = of_clk_get(ddc_node, 1);
	...
	if (!wmixer_node) {
		...
		dp_data->wmixer_info.virt_addr = of_iomap(wmixer_node, index);
		dp_data->wmixer_info.irq_num = irq_of_parse_and_map(wmixer_node, index);
		dp_data->wmixer_info.blk_num = index;
		pr_info("wmixer %d 0x%p  irq:%d\n", index, dp_data->wmixer_info.virt_addr, dp_data->wmixer_info.irq_num);
	}
	...
	if (!rdma_node) {
		...
		for(i = 0; i < RDMA_MAX_NUM; i++) {
			...
			dp_data->rdma_info[i].virt_addr = of_iomap(rdma_node, index);
			dp_data->rdma_info[i].irq_num = irq_of_parse_and_map(rdma_node, index);
	...
	if (!wdma_node) {
		...
		dp_data->wdma_info.virt_addr = of_iomap(wdma_node, index);
		dp_data->wdma_info.irq_num = irq_of_parse_and_map(wdma_node, index);
		dp_data->wdma_info.blk_num = index;
		pr_info("wdma %d 0x%p  irq:%d\n", index, dp_data->wdma_info.virt_addr, dp_data->wdma_info.irq_num);
	}

	pr_info("ddc:%d %d, wmixer:%d rdma:%d wdma:%d\n", dp_data->ddc_info.blk_num, dp_data->DispNum, dp_data->wmixer_info.blk_num, dp_data->rdma_info[RDMA_FB].blk_num, dp_data->wdma_info.blk_num);
	pr_info("ddc:0x%p, wmixer:0x%p rdma:0x%p wdma:0x%p\n", dp_data->ddc_info.virt_addr, dp_data->wmixer_info.virt_addr, dp_data->rdma_info[RDMA_FB].virt_addr, dp_data->wdma_info.virt_addr);
