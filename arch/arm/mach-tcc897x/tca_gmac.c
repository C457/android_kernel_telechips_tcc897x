...
void tca_gmac_portinit(struct gmac_dt_info_t *dt_info, volatile void *ioaddr)
{
	...
	pcfg1 = (dt_info->index == 0) ? (volatile void *)(io_p2v(TCC_PA_HSIOBUSCFG) + GMAC0_CFG1_OFFSET) :
			(dt_info->index == 1) ? (volatile void *)(io_p2v(TCC_PA_HSIOBUSCFG)+ GMAC1_CFG1_OFFSET) : NULL;
...
{
	volatile void *pgpio = (volatile void*)io_p2v(TCC_PA_GPIO);
	unsigned i;
