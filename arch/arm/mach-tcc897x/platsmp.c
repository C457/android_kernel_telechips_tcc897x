...
extern void __cpuinit tcc_cpu_die(unsigned int cpu);
...
{
	int i;
	unsigned smp_cores = readl_relaxed((void __iomem*)sram_p2v(SRAM_BOOT_ADDR+0x60));
...
static void __cpuinit tcc_secondary_init(unsigned int cpu)
{
	write_pen_release(-1);
}

static int __cpuinit tcc_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
